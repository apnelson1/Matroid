...
lemma getElem_of_mem (heF : e ∈ F) : ∃ (i : ℕ) (hi : i < F.length), F[i] = e :=
  List.getElem_of_mem heF

@[simp]
lemma isNonloop_of_mem (heF : e ∈ F) (d : Bool) : (M.bDual d).IsNonloop e := by
  obtain ⟨i, hi, rfl⟩ := getElem_of_mem heF
  exact F.isNonloop

@[simp, grind →]
lemma getElem_inj {i j} {hi : i < F.length} {hj : j < F.length} : F[i] = F[j] ↔ i = j :=
