...
lemma eConn_le_one_of_mem_closure (F : M.Fan)
  ...
  | pair e f b he hf hef =>
    ...
    simpa [getElem_ofPair, sdiff_singleton_eq_self (show e ∉ {f} from hef)] using hcl
  | cons F₀ e heF₀ hT ih =>
...
lemma eConn_eq_zero_of_mem_closure_mem_closure (F : M.Fan) (even : F.length.bodd = false)
