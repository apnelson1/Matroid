...
noncomputable def PolymatroidFn_of_rk (M : Matroid α) (_ : M.Finite): PolymatroidFn M.r where
  submodular := M.rk_submod
  mono := M.rk_mono
  zero_at_bot := M.rk_empty
...
@[simp] theorem sum'_eRk_eq_eRk_sum_on_indep {α ι : Type*} [Fintype ι] [Fintype α]
  ...
  have himage : ∀ i : ι, (fun x : ι × α ↦ x.2) '' (Finset.filter (fun x ↦ f x = i) I.toFinset)
    ...
    · refine fun x hx ↦ ?_
      simp only [Finset.coe_filter, mem_image, mem_ofPred_eq, Prod.exists, exists_eq_right]
  ...
  have : ∀ i, {x_1 | (i, x_1) ∈ I}.ncard =
    ...
    simp only [mem_ofPred_eq, toFinset_setOf]
...
@[simp] theorem sum'_eRk_eq_eRk_sum {α ι : Type*} [Fintype ι] [Fintype α]
  ...
  have : ∀ i : ι, (Ms i).IsBasis' (Prod.mk i ⁻¹' I) (Prod.mk i ⁻¹' X)  := by
    intro i
    simp_all only [IsBasis', Maximal, mem_ofPred_eq, and_imp]
  ...
  simp_rw [← IsBasis'.eRk hI, ← IsBasis'.eRk (this _)]
  exact sum'_eRk_eq_eRk_sum_on_indep hI.1.1

@[simp] theorem sum'_rk_eq_rk_sum {α ι : Type*} [Fintype ι] [Fintype α]
  (Ms : ι → Matroid α) (X : Set (ι × α)):
...
· set N := fun i ↦ (N_singleton Adj i).toFinset
  ...
  have hf_poly : PolymatroidFn f := by
    ...
    · simp only [Finset.inf_eq_inter, Finset.sup_eq_union]
      ...
      refine (le_trans
        ...
        (add_le_add_right  (M.rk_mono ?_) _)) (M.rk_submod _ _))
      simp only [Finset.coe_biUnion, Finset.coe_inter, mem_inter_iff, Finset.mem_coe,
        ...
        mem_ofPred_eq, toFinset_setOf, Finset.coe_filter, Finset.mem_univ, true_and]
  ...
  have heq : ∀ I : Finset β, (ofPolymatroidFn hf_poly).Indep I ↔ (M.adjMap Adj univ).Indep I := by
    ...
    · simp only [Nat.cast_le]
      ...
      · obtain h := Finset.subset_empty.mp (h ▸ hI')
        simp only [h, Finset.card_empty, Finset.biUnion_empty, Finset.coe_empty, rk_empty, le_refl]
      · obtain h | ⟨I₀, f, h', h''⟩ := (AdjIndep.subset ha.1 hI')
        ...
        · obtain h := IsMatching.card_eq h''
          ...
          refine ⟨u, hu, ?_⟩
          simp only [N_singleton, N, mem_ofPred_eq, toFinset_setOf, Finset.mem_filter,
    ...
    · intro v hv
      ...
      specialize hin ⟨v, hv⟩
      simp only [N_singleton, mem_ofPred_eq, toFinset_setOf, Finset.mem_filter,
  ...
  have :  ∀ Y,  f Y = M.r {v | ∃ u ∈ Y, Adj v u} := by
    intro Y
    simp only [N_singleton, mem_ofPred_eq, toFinset_setOf, Finset.coe_biUnion, Finset.mem_coe,
...
theorem adjMap_rank_eq [DecidableEq β] [Fintype α] [Fintype β] (M : Matroid α)
  (Adj : α → β → Prop) :
  (∃ Y , M.r {v | ∃ u ∈ Y, Adj v u} + (Finset.univ \ Y).card ≤ (M.adjMap Adj univ).rank) ∧
  (∀ Y , (M.adjMap Adj univ).rank ≤ M.r {v | ∃ u ∈ Y, Adj v u} + (Finset.univ \ Y).card) := by
...
theorem matroid_partition_eRk' [DecidableEq α] [Fintype α]
  ...
  simp_rw [← cast_rk_eq, ← coe_rank_eq,
...
lemma base_inter_encard_eq [DecidableEq α] {M₁ : Matroid α} {M₂ : Matroid α} {B₁ : Set α} {B₂ : Set α}
  (h₁ : M₁.IsBase B₁) (h₂ : M₂.IsBase B₂) (ground_eq : M₁.E = M₂.E) : (B₁ ∩ B₂).encard + M₂.dual.eRank =
  (B₁ ∪ (M₂.E \ B₂)).encard := by
  rw [← IsBase.encard_compl_eq h₂, ← encard_union_add_encard_inter, inter_assoc, inter_sdiff_self,
    inter_empty, encard_empty, add_zero, inter_union_distrib_right, union_sdiff_self,
    union_eq_self_of_subset_left (Base.subset_ground h₂), inter_eq_self_of_subset_left <|
    union_subset (ground_eq ▸ (Base.subset_ground h₁)) sdiff_subset]


lemma exists_union_isBase_of_indep_union [DecidableEq α] [Fintype α] (Ms : ι → Matroid α)
  : ∃ Is : ι → Set α, (Matroid.Union Ms).IsBase (⋃ i, Is i) ∧ ∀ i, (Ms i).Indep (Is i) := by
  obtain ⟨D, hD⟩ := (Matroid.Union Ms).exists_isBase
  obtain ⟨Is, hIs, hI⟩ := union_indep_iff.mp <| IsBase.indep hD
  exact ⟨Is, hIs ▸ hD, hI⟩

lemma exists_union_isBase_of_isBase_union [DecidableEq α] [Fintype α] (Ms : ι → Matroid α)
  : ∃ Is : ι → Set α, (Matroid.Union Ms).IsBase (⋃ i, Is i) ∧ ∀ i, (Ms i).IsBase (Is i) := by
  classical
  obtain ⟨Is, hIs, hI⟩ := exists_union_isBase_of_indep_union Ms
  obtain ⟨_, hmax⟩ := base_iff_maximal_indep.mp hIs
  choose Is' hIs' hIsub using fun i ↦ (Indep.subset_isBasis'_of_subset (hI i) (subset_iUnion Is i))
  have hB : ∀ i, (Ms i).IsBase (Is' i) := by
    intro i
    rw [isBase_iff_maximal_indep, maximal_subset_iff', and_iff_right (hIs' i).indep]
    contrapose! hmax
    ...
    · contrapose! hne
      simp only [IsBasis', maximal_subset_iff', mem_ofPred] at hIs'
      refine (hIs' i).2 ⟨hI', Function.update_same i I Is' ▸ hf ▸ iUnion_subset_iff.mp hne i ⟩ hsub
  have heq : ⋃ i, Is' i = ⋃ i, Is i := subset_antisymm
    (iUnion_subset_iff.mpr (fun i ↦ IsBasis'.subset <| hIs' i)) (iUnion_mono hIsub)
  exact ⟨Is', heq ▸ hIs, hB⟩


lemma exists_union'_isBase_of_indep_union' [DecidableEq α] [Fintype α] (M : Matroid α)
  (N : Matroid α) : ∃ B C : Set α, (Matroid.union M N).IsBase (B ∪ C) ∧ M.Indep B ∧ N.Indep C:= by
  obtain ⟨D, hD⟩ := (M.union N).exists_isBase
  obtain ⟨B, C, h', hB, hC⟩ := union_indep_iff'.mp <| IsBase.indep hD
  exact ⟨B, C, h' ▸ hD, hB, hC⟩

lemma exists_union'_isBase_of_isBase_union' [DecidableEq α] [Fintype α] (M : Matroid α)
  (N : Matroid α) : ∃ B C : Set α, (Matroid.union M N).IsBase (B ∪ C) ∧ M.IsBase B ∧ N.IsBase C:= by
  classical
  obtain ⟨B, C, h', hB, hC⟩ := exists_union'_isBase_of_indep_union' M N
  obtain ⟨_, hmax⟩ := base_iff_maximal_indep.mp h'
  obtain ⟨B', hB', hBsub⟩ := Indep.subset_isBasis'_of_subset hB (@subset_union_left α B C)
  have hBB': M.IsBase B' := by
    rw [isBase_iff_maximal_indep, maximal_subset_iff', and_iff_right hB'.indep]
    contrapose! hmax
    obtain ⟨I, hI, hsub, hne⟩ := hmax
    refine ⟨I ∪ C, union_indep_iff'.mpr ⟨I, C, rfl, hI, hC⟩, union_subset_union_left C <|
       subset_trans hBsub hsub, ?_⟩
    contrapose! hne
    simp only [IsBasis', maximal_subset_iff', mem_ofPred] at hB'
    refine hB'.2 ⟨hI, subset_union_left.trans hne⟩ hsub
  obtain ⟨C', hC', hCsub⟩ := Indep.subset_isBasis'_of_subset hC (@subset_union_left α C B)
  have hBC': N.IsBase C' := by
    rw [isBase_iff_maximal_indep, maximal_subset_iff', and_iff_right hC'.indep]
    contrapose! hmax
    obtain ⟨I, hI, hsub, hne⟩ := hmax
    refine ⟨B ∪ I, union_indep_iff'.mpr ⟨B, I, rfl, hB, hI⟩, union_subset_union_right B <|
       subset_trans hCsub hsub, ?_⟩
    contrapose! hne
    simp only [IsBasis', maximal_subset_iff', mem_ofPred] at hC'
  ...
  have heq : B' ∪ C' = B ∪ C := subset_antisymm (union_subset_iff.mpr ⟨Basis'.subset hB',
    union_comm B C ▸ IsBasis'.subset hC'⟩) (union_subset_union hBsub hCsub)
...
theorem matroid_intersection_aux [Fintype α] (M : Matroid α) (N : Matroid α) (ground_eq : M.E = N.E)
    ...
    M.eRk X + N.eRk Xᶜ := by
  classical
  obtain ⟨B, C, h, hB, hC⟩ := exists_union'_isBase_of_isBase_union' M N.dual
  obtain ⟨X, heq⟩ := matroid_partition_eRk' M N.dual
  obtain h' := base_inter_encard_eq hB (Base.compl_isBase_of_dual hC) ground_eq ▸ dual_ground ▸
    sdiff_sdiff_cancel_left (N.dual.subset_ground C hC) ▸ heq ▸ IsBase.encard h
  obtain hX := encard_sdiff_add_encard_of_subset <| subset_univ X

  obtain hd := ground_univ ▸ N.dual_eRk_add_eRank X
  obtain hg := ground_univ ▸ N.eRank_add_dual_eRank.symm
  refine ⟨B ∩ (N.E \ C), X, Matroid.Indep.subset (Matroid.IsBase.indep hB) inter_subset_left,
    Matroid.Indep.subset (Matroid.IsBase.indep (Base.compl_isBase_of_dual hC)) inter_subset_right, ?_⟩
  have hcard : (B ∩ (N.E \ C)).encard + N✶.eRank + X.encard + N.eRank =
    M.eRk X + N✶.eRk X + (univ \ X).encard + X.encard + N.eRank := by
    rw [h']
  -- have aux : _ + X.encard + (N.eRank + N✶.eRank)
  --   = M.eRk X + (univ \ X).encard + X.encard + (N✶.eRk X + N.eRank) := by
  --   convert hcard using 1 <;> ac_rfl
  -- rw [N.dual_eRk_add_eRank, eRank_add_dual_eRank] at aux

  rwa [add_comm, add_comm (B ∩ (N.E \ C)).encard, ← add_assoc, ← add_assoc, ← hg, add_assoc (M.eRk X),
     add_comm (N✶.eRk X), add_assoc (M.eRk X), add_assoc (univ \ X).encard, add_comm (N✶.eRk X),
     add_assoc (M.eRk X), add_assoc (univ \ X).encard, add_assoc X.encard, hd,
     ← add_assoc (univ \ X).encard, hX, ← add_assoc univ.encard, add_comm (M.eRk X),
     add_assoc, add_assoc, add_assoc, WithTop.add_left_cancel_iff, add_comm (a := X.encard),
     ← add_assoc, WithTop.add_right_cancel_iff, add_comm, ← compl_eq_univ_sdiff] at hcard
...
theorem exists_common_ind (M₁ M₂ : Matroid α) [M₁.Finite] :
    ∃ I X, M₁.Indep I ∧ M₂.Indep I ∧ I.encard = M₁.eRk X + M₂.eRk (M₂.E \ X) := by
  suffices aux : ∀ (E : Set α) (M₁ M₂ : Matroid α), M₁.Finite → M₁.E = E → M₂.E = E →
      ∃ I X, X ⊆ M₁.E ∩ M₂.E ∧  M₁.Indep I ∧ M₂.Indep I ∧ I.encard = M₁.eRk X + M₂.eRk (E \ X) by
    obtain ⟨I, X, _, hI₁, hI₂, hIX⟩ := aux M₁.E M₁ (M₂ ↾ M₁.E) (by assumption) rfl rfl
    simp only [restrict_eRk_eq', inter_eq_self_of_subset_left sdiff_subset] at hIX
    refine ⟨I, X ∪ (M₂.E \ M₁.E), hI₁, hI₂.of_restrict, ?_⟩
    rwa [← eRk_inter_ground (X := _ ∪ _), union_inter_distrib_right, inter_comm (a := _ \ _),
      inter_sdiff_self, union_empty, eRk_inter_ground, ← sdiff_sdiff, sdiff_sdiff_comm,
      sdiff_sdiff_right_self, inter_sdiff_assoc, inter_comm, eRk_inter_ground]
  clear! M₁ M₂
  ...
  simp only [restrictSubtype, eRk_comap_eq, restrict_eRk_eq', image_val_inter_self_left_eq_coe,
    image_val_compl, inter_eq_self_of_subset_left sdiff_subset] at hIX
  ...
  --   rw [rk_eq_rk_inter_ground, union_inter_distrib_right, sdiff_inter_self, empty_union,
  --     ← rk_eq_rk_inter_ground, ← sdiff_sdiff, sdiff_sdiff_right_self, sdiff_eq, inter_assoc]
  --   sorry
  -- intro M₂' h_eq
