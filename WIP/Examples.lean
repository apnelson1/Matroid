...
def M_K₄ : Matroid (Fin 6) := by
  ...
  set rank := 3
  set E := @univ (Fin 6)
  set Circuit := fun C ↦ NonspanningCircuit C ∨ C.card = rank + 1 ∧ (∀ ⦃C'⦄, NonspanningCircuit C' → ¬C' ⊆ C) ∧ ↑C ⊆ E
  ...
  exact FinsetNonspanningCircuitMatroid.matroid <| FinsetNonspanningCircuitMatroid.mk
    ...
    (exists_NonspanningCircuitless_rank_set := by exact ⟨{0, 1, 3}, by simp, by simp; tauto⟩)
      -- uhmmm long simps
...
def W3 :  Matroid (Fin 6) := by
  ...
  exact FinsetNonspanningCircuitMatroid.matroid <| FinsetNonspanningCircuitMatroid.mk
    ...
    (NonspanningCircuit_subset_ground := by simp only [Fin.isValue, mem_insert_iff, mem_singleton_iff,
      subset_univ, implies_true])
    (exists_NonspanningCircuitless_rank_set := by exact ⟨{0, 1, 3}, by simp only [Fin.isValue,
