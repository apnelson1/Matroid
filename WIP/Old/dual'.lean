  ...
  matroid_of_isBase
    ...
    (by
      ...
      rw [diff_diff_singleton (show x ∈ M.E \ B₂ from ⟨hxE, hx.2⟩),
        diff_diff, insert_union,
        union_eq_self_of_subset_left (diff_subset_diff_right subset_union_right)] at hB''₂
      have hne : M.E \ B₁ ≠ B''
      · rintro rfl
...
theorem dual_indep_iff_exists' : (M✶.Indep I) ↔ I ⊆ M.E ∧ (∃ B, M.IsBase B ∧ Disjoint I B) := by
  ...
  exact ⟨fun ⟨B, hB, hIB⟩ ↦
    ...
    fun ⟨hIE, B, hB, hdj⟩ ↦ ⟨M.E \ B, ⟨⟨by rwa [diff_diff_cancel_left hB.subset_ground],
      diff_subset _ _⟩, subset_diff.2 ⟨hIE, hdj⟩⟩⟩⟩
