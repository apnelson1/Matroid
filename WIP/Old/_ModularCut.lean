...
theorem IsBasis.exchange_isBase_of_indep {M : Matroid α} {f e : α} {X : Set α} (hB : M.IsBasis B X) (hf : f ∈ X \ B)
  ...
  · apply hB.exchange_isBase_of_indep hf.2 (hI.indep_restrict_of_subset (insert_subset hf.1
    (sdiff_subset.trans hB.subset_ground)))
...
lemma modular_finite_intersection {M : Matroid α} {X : Set (Set α)} {Fs : Set (Set α)}
  ...
  · apply hfin.encard_lt_encard ⟨sdiff_subset _ _, (not_subset.2 ⟨x, (xy_sub (mem_insert _ _)), _⟩)⟩
    exact fun x_mem ↦ absurd rfl x_mem.2
  have:= encard_lt
  apply modular_finite_intersection forall_isFlat pair_modular (hfin.subset sdiff_subset) (sdiff_subset.trans
   hsub) (IsModularFamily.subset ⟨B, B_mod⟩ sdiff_subset) ⟨y, ⟨(pair_subset_iff.1 xy_sub).2, _⟩⟩
  exact xy_ne.symm
...
theorem ModularCut.remove {M : Matroid α} (C : M.ModularCut) {B Y : Set α}
  ...
  dsimp
  rw [B'_isBase.indep.closure_inter_eq_self_of_subset (sdiff_subset.trans B_sub_B')]
  apply (hB₁.subset (sdiff_subset _ {y'})).isBasis_closure
...
@[simps] def ModularCut.extensionIndepMatroid (C : M.ModularCut) (e : α) :
  ...
  indep_subset I J := by
    ...
    by_cases heI : e ∈ I
    · refine Or.inr ⟨⟨heI, heE⟩, hJ.subset (sdiff_subset_sdiff_left hIJ), fun hIC ↦ hJC ?_⟩
      exact C.superset hIC (M.closure_subset_closure (sdiff_subset_sdiff_left hIJ)) (M.closure_isFlat _)
    refine Or.inl (hJ.subset <| subset_sdiff_singleton hIJ heI)
  indep_aug := by
    -- hard work here
      ...
      ·  --e in neither case
        ...
        · intro h_f
          ...
          · apply (ne_insert_of_notMem J e_in_J (subset_antisymm (subset_insert e J) (J_max _ (subset_insert e J))))
            ...
            rwa [insert_sdiff_self_of_notMem e_in_J, (J_isBase_of_ind J_ind).closure_eq]
          rw [spanning_iff_superset_isBase (Y_ind.subset_ground)]
          refine' ⟨I, h_f, subset_sdiff_singleton I_sub_Y (notMem_subset I_ind.subset_ground e_in_Y.2)⟩
      ...
      · by_contra! h_f
        ...
        · obtain ⟨y, hy⟩ := Set.exists_of_ssubset (ssubset_iff_subset_ne.2 ⟨I_sub_Y, I_ne_Y⟩)
          ...
          · have y_insert_indep : M.Indep (insert y (insert i (J \ {e})))
            · rw [i_ind.insert_indep_iff_of_notMem, mem_sdiff]
            ...
            · by_contra insert_notMem
              ...
              rw [insert_sdiff_singleton_comm (ne_of_mem_of_notMem mem_c.1 e_in_J.2)]
              apply sdiff_subset_sdiff_left (J_max (Or.inr _) (subset_insert _ _))
              rw [← insert_sdiff_singleton_comm (ne_of_mem_of_notMem mem_c.1 e_in_J.2)]
            ...
            · by_contra insert_notMem
              ...
              rw [insert_sdiff_singleton_comm (ne_of_mem_of_notMem (I_ind.subset_ground i_mem.1) e_in_J.2)]
              · apply sdiff_subset_sdiff_left (J_max (Or.inr _) (subset_insert _ _))
                rw [← insert_sdiff_singleton_comm (ne_of_mem_of_notMem (I_ind.subset_ground i_mem.1) e_in_J.2)]
          ...
          exact hy.2 y_in_I
        apply Y_closure_notMem (C.superset closure_I_mem (M.closure_subset_closure (subset_sdiff_singleton I_sub_Y e_notin_I)) (M.closure_isFlat _))
      refine' ⟨e, ⟨e_in_J.1, e_notin_I⟩, Or.inr ⟨⟨mem_insert _ _, e_in_J.2⟩, _, _⟩⟩
      · rwa [insert_sdiff_self_of_notMem e_notin_I]
      rwa [insert_sdiff_self_of_notMem e_notin_I]
    obtain (J_ind | ⟨e_in_J, J_ind, J_closure_notMem⟩) := J_ind
    ...
    · obtain (Y_ind | ⟨e_in_Y, Y_ind, Y_closure_notMem⟩) := Y_ind
      ...
      · intro I_Base
        ...
        refine' ⟨sdiff_subset_sdiff_left I_sub_Y, fun sdiff_eq ↦ _⟩
        apply I_ne_Y (subset_antisymm I_sub_Y _)
        intro y y_in_Y
        obtain (rfl | hne) := eq_or_ne y e
        · exact e_in_I.1
        apply sdiff_subset I {e}
        rw [sdiff_eq]
        exact ⟨y_in_Y, hne⟩
      obtain ⟨j₂, hj₂⟩ := I_ind.exists_insert_of_not_isBase I_nb (J_isBase_of_ind J_ind)
      rw [sdiff_sdiff_right, Disjoint.inter_eq (disjoint_singleton_right.2 (notMem_subset J_ind.subset_ground e_in_Y.2)),
       union_empty] at hj₂
      by_cases j₂I_b : M.IsBase (insert j₂ (I \ {e}))
      · refine' ⟨j₂, hj₂.1, _⟩
        dsimp
        rw [← insert_sdiff_singleton_comm (ne_of_mem_of_notMem (J_ind.subset_ground hj₂.1.1)
        ...
        · exact Or.inr ⟨⟨mem_insert_of_mem _ e_in_I.1, e_in_I.2⟩, fun closure_mem ↦ Y_closure_notMem
           ...
           (ne_of_mem_of_notMem e_in_I.1 hj₂.1.2).symm⟩ (sdiff_subset_sdiff_left I_sub_Y)))
           (M.closure_isFlat _))⟩
        have y_notin : y ∉ insert j₂ (I \ {e})
        · rw [mem_insert_iff, not_or, mem_sdiff, not_and_or]
          exact ⟨ne.symm, Or.inl hy.2⟩
        have base_insert:= @Matroid.IsBase.exchange_isBase_of_indep _ _ _ _ j₂ j₂I_b y_notin
        rw [insert_sdiff_self_of_notMem (notMem_subset sdiff_subset hj₂.1.2)] at base_insert
        rw [j₂I_b.closure_eq]
        rw [Spanning.closure_eq _] at Y_closure_notMem
        · refine Or.inr ⟨⟨mem_insert_of_mem _ e_in_I.1, e_in_I.2⟩, Y_closure_notMem⟩
        have insert_y_subset_Y : insert y (I \ {e}) ⊆ Y \ {e}
        · rw [insert_sdiff_singleton_comm (ne_of_mem_of_notMem e_in_I (notMem_subset sdiff_subset hy.2)).symm]
          exact sdiff_subset_sdiff_left (insert_subset hy.1 I_sub_Y)
      ...
      · by_cases j₂_closure_mem_c : M.closure (insert j₂ (I \ {e})) ∈ C
        ...
        refine' ⟨j₂, hj₂.1, _⟩
        dsimp
        rw [← insert_sdiff_singleton_comm
      ...
      dsimp
      rw [ ← insert_sdiff_singleton_comm (ne_of_mem_of_notMem j₁_mem.1 (notMem_subset J_ind.subset_ground e_in_I.2)) _]
    ...
    · intro x x_notin_J x_ind
      ...
      apply (ne_insert_of_notMem _ x_notin_J) (subset_antisymm (subset_insert _ _) (J_max _
        (subset_insert _ _)))
      dsimp
      rw [← insert_sdiff_singleton_comm (ne_of_mem_of_notMem
    ...
    · refine' ⟨Y_ind.subset (sdiff_subset_sdiff_left (insert_subset hy.1 I_sub_Y)),
        fun closure_mem_C ↦ _⟩
      exact Y_closure_notMem (C.superset closure_mem_C (M.closure_subset_closure (sdiff_subset_sdiff_left (insert_subset
    ...
    have y_ground := (Y_ind.subset_ground (mem_diff_of_mem hy.1 (ne_of_mem_of_notMem e_in_J
      (notMem_subset sdiff_subset y_notin_J)).symm))
    have x := I_ind.subset_isBasis_of_subset (sdiff_subset_sdiff_left (subset_union_left I J)) ?_
    · obtain ⟨I', I'_isBasis, I_sub_I'⟩ := x
      have : (I \ {e} ⊂ I')
      · have hne:= (ne_of_mem_of_notMem e_in_J (notMem_subset (sdiff_subset _ M.E) y_notin_J)).symm
        ...
        refine' Or.inr ⟨⟨(mem_insert_of_mem _ e_in_J.1), e_in_J.2⟩, notMem_subset (M.closure_subset_closure (sdiff_subset_sdiff_left
          (subset_union_right I J))) _, _⟩
        · rw [← I'_isBasis.closure_eq_closure, I_ind.notMem_closure_iff_of_notMem (notMem_subset
          sdiff_subset hy.2),
          insert_sdiff_singleton_comm hne]
          exact y_ind.1
        rw [insert_sdiff_singleton_comm hne]
        intro closure_mem_C
        apply y_ind.2 (C.superset closure_mem_C _ (M.closure_isFlat _))
        rw [← insert_sdiff_singleton_comm hne I,
          ← closure_insert_closure_eq_closure_insert, I'_isBasis.closure_eq_closure, closure_insert_closure_eq_closure_insert,
          ← insert_sdiff_singleton_comm hne]
        · apply M.closure_subset_closure (insert_subset_insert (sdiff_subset_sdiff_left
          subset_union_right))
        exact notMem_subset sdiff_subset y_notin_J
      ...
      · intro J_isBasis
        ...
        rw [← insert_sdiff_singleton_comm, I'_isBasis.closure_eq_closure, ← J_isBasis.closure_eq_closure]
      ...
      obtain ⟨i, hi⟩ := J_ind.exists_insert_of_not_isBasis (sdiff_subset_sdiff_left
      ...
      · by_contra I'_not_isBase
        ...
        · intro b_mem_union
          have : b ∈ (I ∪ J) \ {e}
          · rwa [mem_sdiff_singleton, and_iff_left (ne_of_mem_of_notMem
        ...
        · rw [hi.2.insert_indep_iff, mem_sdiff _, and_iff_right (hB.subset_ground hb.1.1)]
          ...
          rw [insert_sdiff_singleton_comm (ne_of_mem_of_notMem (hi.2.subset_ground
            (mem_insert _ _)) e_in_I.2)]
          apply sdiff_subset_sdiff_left (insert_subset _ subset_union_right)
      ...
      · have base_insert:= @Matroid.IsBase.exchange_isBase_of_indep _ _ _ y a I'_isBase
        rw [insert_sdiff_self_of_notMem a_notMem] at base_insert
        apply base_insert
        · rintro (rfl | y_mem)
          · exact y_notin_J a_mem_JI.1
          exact hy.2 y_mem.1
        rw [insert_sdiff_singleton_comm (ne_of_mem_of_notMem y_ground e_in_I.2)]
        exact y_ind.1
      apply h_f a a_mem_JI
      rw [← insert_sdiff_singleton_comm
        (ne_of_mem_of_notMem e_in_I (notMem_subset sdiff_subset a_mem_JI.2)).symm,
        I'_isBase.closure_eq, ← yI_isBase.closure_eq, insert_sdiff_singleton_comm (ne_of_mem_of_notMem y_ground e_in_I.2)]
  ...
  indep_maximal := by
    ...
    · by_cases e_in_Y : e ∈ Y
      ...
      · obtain (X_ground | ⟨e_mem, Xdiff_sub⟩) := subset_insert_iff.1 X_sub
        ...
        obtain ⟨B, B_Basis, Y_sub_B⟩ := Y_ind.subset_isBasis_of_subset (subset_sdiff_singleton Y_sub_X e_in_Y) Xdiff_sub
        by_cases eB_ind : M.Indep (insert e B)
        · refine' ⟨insert e B, mem_maximals_iff.2 ⟨⟨Or.inl eB_ind, Y_sub_B.trans (subset_insert _ _)
            , insert_subset e_in_X (B_Basis.subset.trans sdiff_subset)⟩, fun I I_ind I_sub ↦ _⟩⟩
          obtain ⟨(I_ind | ⟨e_in_I, _, _⟩), Y_sub_I, I_sub_X⟩ := I_ind
          · obtain (rfl | ssub) := eq_or_ssubset_of_subset I_sub
            · rfl
        ...
        · refine' ⟨B, mem_maximals_iff.2 ⟨⟨Or.inl B_Basis.indep, Y_sub_B, (B_Basis.subset.trans sdiff_subset)⟩, fun I I_ind I_sub ↦ _⟩⟩
          obtain ⟨(I_ind | ⟨e_in_I, _, closure_notMem⟩), Y_sub_I, I_sub_X⟩ := I_ind
          · obtain (rfl | ssub) := eq_or_ssubset_of_subset I_sub
            · rfl
            have I_sub_Xd := subset_sdiff_singleton I_sub_X (fun (e_mem : e ∈ I) ↦ (eB_ind (I_ind.subset (insert_subset e_mem I_sub))))
            apply absurd I_ind (B_Basis.dep_of_ssubset ssub I_sub_Xd).not_indep
          obtain (closure_mem_C | e_mem) := closure_mem_C
          · apply absurd (C.superset closure_mem_C (M.closure_subset_closure (subset_sdiff_singleton I_sub
        ...
        refine' ⟨insert e B, mem_maximals_iff.2 ⟨⟨Or.inr ⟨⟨mem_insert _ _, closure_mem_C.2⟩, by simp
         ...
         insert_subset e_in_X (B_Basis.subset.trans sdiff_subset)⟩, fun I I_ind sub_I ↦ _⟩⟩
        ...
        · rintro (rfl | e_B)
          ...
          exact e_nB e_B
        have iB_sub_Xd : insert i B ⊆ X \ {e}:= subset_sdiff_singleton (insert_subset (I_sub_X hi.1) ((B_Basis.subset).trans sdiff_subset)) e_niB
        ...
        exact subset_sdiff_singleton (insert_subset hi.1 ((subset_insert _ _).trans sub_I)) e_niB
      rw [← sdiff_singleton_subset_iff, sdiff_singleton_eq_self e_in_X] at X_sub
    ...
    rw [← sdiff_singleton_subset_iff] at X_sub
    obtain ⟨B, B_Basis, Y_sub_B⟩ := Y_ind.subset_isBasis_of_subset (sdiff_subset_sdiff_left Y_sub_X) X_sub
    have e_nB : e ∉ B
    ...
    · refine' ⟨insert e B, mem_maximals_iff.2 ⟨⟨Or.inr ⟨⟨mem_insert _ _, e_in_Y.2⟩, by simp
       [B_Basis.indep, e_nB], by simp [closure_B_mem_C, e_nB]⟩, sdiff_singleton_subset_iff.1 Y_sub_B,
       insert_subset (Y_sub_X e_in_Y.1) (B_Basis.subset.trans sdiff_subset)⟩, fun I I_ind sub_I ↦ _⟩⟩
      obtain ⟨(I_ind | ⟨e_in_I, I_ind, _⟩), _, I_sub_X⟩ := I_ind
      · exact absurd (I_ind.subset_ground (sub_I (mem_insert _ _))) e_in_Y.2
      obtain (rfl | ssub) := eq_or_ssubset_of_subset (subset_sdiff_singleton ((subset_insert _ _).trans sub_I) e_nB)
      · simp [e_in_I.1]
      apply absurd I_ind (B_Basis.dep_of_ssubset ssub (sdiff_subset_sdiff_left I_sub_X)).not_indep
    ...
    refine' ⟨(insert e (B \ {b})), mem_maximals_iff.2 ⟨⟨Or.inr ⟨⟨mem_insert _ _, e_in_Y.2⟩, by simp
     [e_nB, B_Basis.indep.subset (sdiff_subset B {b})], by simp [e_nB, b_closure_mem]⟩,
     ⟨sdiff_singleton_subset_iff.1 (subset_sdiff_singleton Y_sub_B b_mem.2), insert_subset (Y_sub_X
     e_in_Y.1) ((sdiff_subset.trans B_Basis.subset).trans sdiff_subset)⟩⟩,
     fun I I_ind sub_I ↦ _⟩⟩
    ...
    · apply insert_subset _ _
      ...
      apply subset_sdiff_singleton ((subset_insert _ _).trans sub_I) (notMem_subset
       (sdiff_subset.trans (B_Basis.left_subset_ground)) e_in_Y.2)
    have iBb_C : M.closure (insert i (B \ {b})) ∈ C
    · obtain (rfl | i_ne_b) := eq_or_ne i b
      · simp only [b_mem.1, not_true_eq_false, mem_sdiff, mem_singleton_iff, and_false,
        insert_sdiff_singleton, insert_eq_of_mem, closure_B_mem_C]
      rwa [@Basis.closure_eq_closure _ M _ (X \ {e}) _, ← B_Basis.closure_eq_closure]
      apply B_Basis.exchange_isBase_of_indep
      rw [mem_sdiff, mem_sdiff, mem_singleton_iff]
  ...
  subset_ground := by
    ...
    simpa using sdiff_subset_iff.1 hI.subset_ground
...
theorem ModularCut.extension_closure_eq_of_mem {e : α} {X : Set α} (C : M.ModularCut) (he : e ∉ M.E) (hX : M.closure X ∈ C) :
  ...
  · rw [isBasis'_iff_isBasis_inter_ground, C.extension_ground, basis_iff, C.extension_indep,
    ...
    exact absurd hX (fun I_closure_mem ↦ closure_notMem (C.superset I_closure_mem (M.closure_subset_closure
     (subset_sdiff_singleton I_sub_J (notMem_subset hI.1.subset_ground e_mem.2))) (M.closure_isFlat _)))
  ...
  · obtain (x_dep | x_mem) := ex_isBasis.indep.mem_closure_iff.1 x_mem
    · have x_ground:= x_dep.subset_ground
      ...
      rw [C.extension_ground e, ←sdiff_singleton_subset_iff, sdiff_singleton_eq_self _] at x_ground
      · assumption
      rintro (rfl | e_I)
      · exact ne rfl
  ...
  · have xnI := (notMem_subset hI.indep.subset_ground he)
    ...
    rwa [insert_sdiff_self_of_notMem xnI, hI.closure_eq_closure]
  ...
  · obtain (x_dep | x_mem) := ex_isBasis.indep.mem_closure_iff.1 x_mem
    · have x_ground:= x_dep.subset_ground
      ...
      · rw [dep_iff, (C.extension_indep x), not_or] at x_dep
        ...
        rw [insert_sdiff_self_of_notMem (notMem_subset hI.indep.subset_ground he), hI.closure_eq_closure]
...
theorem ModularCut.extension_isFlat_iff {F : Set α} {C : M.ModularCut} (hC : C.Nonempty) (e : α) (he : e ∉ M.E) :
  ...
  · obtain (e_mem | e_notMem) := em (e ∈ F ∧ F \ {e} ∈ C)
    ...
    · have ground_eq : M.E = (C.extension e).E \ {e}
      · rw [(C.extension_ground), insert_sdiff_self_of_notMem he]
      rw [ground_eq, ←delete]
      exact C.extension_delete he
    obtain (e_notMem | f_notMem) := not_and_or.1 e_notMem
    · have Fsub : F ⊆ M.E
      · rw [←sdiff_singleton_eq_self e_notMem, sdiff_singleton_subset_iff, ←closure_eq_self, ←(C.extension_ground)]
      ...
      (C.extension_indep e), not_or, insert_sdiff_self_of_notMem e_nI, hI'.closure_eq_closure,
...
@[simps] def ModularCut.ofDelete (M : Matroid α) (e : α) : (M ＼ e).ModularCut where
  ...
  forall_isFlat := by simp only [deleteElem, mem_ofPred_eq, and_imp]; tauto
  up_closed := by
    simp only [deleteElem, mem_ofPred_eq, and_imp]
...
def ModularCut.extensionEquiv (M : Matroid α) (e : α) (he : e ∉ M.E) :
  ...
  left_inv := by
    ...
    ext F
    simp only [deleteElem, mem_ofPred_eq, extension_ground, congr_Fs, SetLike.mem_coe,
    ...
    · obtain ⟨B, hB⟩ := M.exists_isBasis F F_IsFlat.1.subset_ground
      ...
      · rw [←(@extension_delete _ M C e he)] at hB
        exact ((basis_restrict_iff sdiff_subset).1 hB).1
      ...
      insert_sdiff_self_of_notMem e_not_B, or_iff_right (not_not.2 hB.indep),
    ...
    not_or, not_and_or, not_and_or, insert_sdiff_self_of_notMem e_not_B, not_not, hB.closure_eq_closure,
  ...
  right_inv := by
    rintro ⟨N, hN, rfl⟩
    simp only [deleteElem, coe_setOf, mem_ofPred_eq, Subtype.mk.injEq]
    apply ext_indep (by simpa) (fun I hI ↦ ?_)
    by_cases heI : e ∈ I
    · simp only [deleteElem, mem_ofPred_eq, extension_indep, delete_indep_iff,
        disjoint_singleton_right, heI, not_true_eq_false, and_false, delete_ground, mem_sdiff,
      ...
      refine' ⟨_, fun ind ↦ ⟨ind.subset sdiff_subset, _⟩⟩
      · rintro ⟨ind, isFlat⟩
        have Ie_isBasis : N.IsBasis (I \ {e}) (N.closure (I \ {e}) \ {e})
        · exact (ind.isBasis_closure).isBasis_subset (subset_sdiff_singleton (N.subset_closure _) (fun e_mem ↦ absurd rfl e_mem.2)) sdiff_subset
        ...
        · exact (basis_restrict_iff sdiff_subset).2 ⟨Ie_isBasis, (sdiff_subset_sdiff_left (N.closure_subset_ground _))⟩
        ...
        · rw [ind.mem_closure_iff, dep_iff]
          ...
          · obtain (a_dep | a_mem) := a_mem
            · exact Or.inl ⟨a_dep.1, a_dep.2.trans sdiff_subset⟩
            exact Or.inr a_mem
          exact subset_sdiff_singleton ind.subset_ground (fun e_mem ↦ e_mem.2 rfl)
