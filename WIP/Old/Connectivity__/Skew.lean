...
lemma Skew.contract_subset_union {C : Set α} (hXY : M.Skew X Y) (hC : C ⊆ X ∪ Y) :
  ...
  grw [contract_skew_iff disjoint_sdiff_left disjoint_sdiff_left, sdiff_union_self, sdiff_union_self,
...
lemma Coindep.skew_compl_iff_subset_loops (hX : M.Coindep X) :
  ...
  refine ⟨fun h ↦ ?_, fun h ↦ skew_of_subset_loops h sdiff_subset⟩
  ...
  rw [union_sdiff_cancel_right (disjoint_sdiff_right.mono hI.subset hB.subset).inter_eq.subset,
    sdiff_self] at hIB
