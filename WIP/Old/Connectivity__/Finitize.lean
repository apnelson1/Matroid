...
lemma IsBase.exists_preserve_eConn_delete (hB : M.IsBase B) (hk : k ≤ M.eConn X) :
  ...
  refine ⟨R \ B, sdiff_subset.trans hRE, disjoint_sdiff_left, hRK, ?_⟩
  ...
  rwa [sdiff_sdiff_right, disjoint_sdiff_left.inter_eq, union_empty, sdiff_sdiff, delete_compl,
...
lemma IsBase.exists_preserve_eConn_delete_contract (hB : M.IsBase B) (hk : k ≤ M.eConn X) :
  ...
  have hB' : (M ＼ ((M.E \ B) \ V)).IsBase B := by
    rw [delete_isBase_iff, sdiff_sdiff_right, sdiff_sdiff_cancel_left hB.subset_ground]
  ...
  have hrw : (M ＼ ((M.E \ B) \ V))✶.E = B ∪ V := by
    rw [dual_ground, delete_ground, sdiff_sdiff_right, sdiff_sdiff_cancel_left hB.subset_ground,
      inter_eq_self_of_subset_right hVE]
  simp only [dual_delete, contract_ground, dual_ground, sdiff_sdiff_right, sdiff_sdiff_right_self,
    inf_eq_inter, inter_eq_self_of_subset_right hB.subset_ground, inter_eq_self_of_subset_right hVE,
    delete_ground, union_sdiff_left, union_sdiff_right,
    inter_eq_self_of_subset_right subset_union_left, union_eq_self_of_subset_left sdiff_subset]
    at hUconn
  simp only [delete_ground, sdiff_sdiff_right, sdiff_sdiff_right_self, inf_eq_inter,
    inter_eq_self_of_subset_right hB.subset_ground, inter_eq_self_of_subset_right hVE,
    union_sdiff_left, hVB.sdiff_eq_left] at Udj
  rw [hrw, union_comm,  ← sdiff_subset_iff, Udj.sdiff_eq_left] at hUE
...
lemma IsBase.exists_preserve_eConn_delete_of_le (hB : M.IsBase B) (hk : k ≤ M.eConn X) :
  ...
  grw [← hCconn]
  exact (delete_isRestriction_of_subset _ (subset_sdiff.2 ⟨hD, hDV⟩)).isMinor.eConn_le _
...
lemma IsBase.exists_preserve_eConn_contract_delete_of_le (hB : M.IsBase B) (hk : k ≤ M.eConn X) :
  ...
  have hDss : D ⊆ (M.E \ B) \ V := subset_sdiff.2 ⟨hD, hDV⟩
  refine (delete_isRestriction_of_subset _ hDss).isMinor.trans (IsMinor.delete_isMinor_delete ?_ ?_)
  · exact contract_isMinor_of_subset _ (subset_sdiff.2 ⟨hC, hCU⟩)
  grw [hD, contract_ground, sdiff_subset_sdiff_right sdiff_subset]
