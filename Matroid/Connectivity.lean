...
lemma Indep.encard_inter_add_erk_dual_eq_of_closure_eq_closure (hI : M.Indep I) (hI' : M.Indep I')
  ...
  have hb : ∀ ⦃B⦄, M.Basis B (M.closure I) → (M ／ (K \ I)).Basis B (B ∪ J \ (K \ I)) ∧ (K \ I ⊆ B ∪ J) := by
    ...
    · rw [contract_closure_eq, subset_diff, disjoint_union_left, and_iff_left disjoint_sdiff_left,
        and_iff_left (hdj.mono_left hB.subset), ← closure_union_closure_left_eq, hB.closure_eq_closure, closure_closure,
        closure_union_closure_left_eq, union_diff_self, union_eq_self_of_subset_left hIK, hK.closure_eq_closure]
