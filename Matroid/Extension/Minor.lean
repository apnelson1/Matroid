...
protected def project (U : M.ModularCut) (C : Set α) : (M.project C).ModularCut where
  ...
  forall_inter Fs hFs hne hmod := by
    ...
    rw [← project_inter_ground] at hmod
    exact hmod.of_project (fun F ↦ (hFs F.2).2)

@[simp]
lemma mem_project_iff (U : M.ModularCut) (C : Set α) (hC : C ⊆ M.E := by aesop_mat) :
    F ∈ U.project C ↔ F ∈ U ∧ C ⊆ F := by
  simp [ModularCut.project, inter_eq_self_of_subset_left hC]

lemma mem_project_iff' (U : M.ModularCut) (C : Set α) : F ∈ U.project C ↔ F ∈ U ∧ C ∩ M.E ⊆ F :=
  Iff.rfl

@[simp]
lemma principal_project (M : Matroid α) (X C : Set α) :
    (ModularCut.principal M X).project C = ModularCut.principal (M.project C) (X ∪ C) := by
  refine ModularCut.ext fun F hF ↦ ?_
  rw [mem_project_iff', mem_principal_iff', mem_principal_iff', project_isFlat_iff']
  grind

/-- Given a modular cut `U` of `M`, the corresponding modular cut in some contraction of `M`. -/
...
lemma mem_contract_iff (U : M.ModularCut) (C : Set α) :
  ...
  have aux : C ∩ M.E ⊆ M.closure (F ∪ C) := by
    grw [M.inter_ground_subset_closure, ← subset_union_right]
  simp [isFlat_contract_iff', ModularCut.contract, ModularCut.mem_project_iff', aux]
...
lemma projectBy_project (U : M.ModularCut) (C : Set α) :
  ...
  have aux {X} : C ∩ M.E ⊆ M.closure (X ∪ C) := by
    grw [M.inter_ground_subset_closure, ← subset_union_right]
  simp [Set.ext_iff, mem_closure_projectBy_iff, U.mem_project_iff', aux, ← insert_union]
...
lemma project_eq_top_iff (U : M.ModularCut) : U.project X = ⊤ ↔ M.closure X ∈ U := by
  rw [ModularCut.eq_top_iff, project_loops, mem_project_iff',
    and_iff_left (inter_ground_subset_closure ..)]
...
lemma exists_splice_of_contract_eq_deleteElem (heC : e ∉ C) (h_eq : M ／ C = N ＼ {e}) :
  ...
  set UM := (UN.copy h_eq.symm).ofContract hCE
  use M.extendBy e UM
...
lemma exists_splice_of_delete_eq_contractElem (heD : e ∉ D) (h_eq : M ＼ D = N ／ {e}) :
  ...
  -- have := exists_splice_of_contract_eq_deleteElem (by simpa) (by simpa) h_eq
  obtain ⟨P, himp, hPM, hPN⟩ :=
    exists_splice_of_contract_eq_deleteElem (by simpa) h_eq
  rw [eq_dual_iff_dual_eq] at hPM hPN
  refine ⟨P✶, himp, by simpa using hPM, by simpa using hPN⟩

/-- If the delete-set is finite and disjoint from the delete-set,
then any two matroids with a common minor have a splice. -/
lemma exists_splice_of_contract_eq_delete (hCD : Disjoint C D) (hDfin : D.Finite)
    (h_eq : M ／ C = N ＼ D) : ∃ (P : Matroid α), (D ⊆ N.E → D ⊆ P.E) ∧ P ＼ D = M ∧ P ／ C = N := by
  induction D, hDfin using Set.Finite.induction_on generalizing N with
  | empty => simpa using h_eq
  | @insert a D has hs IH =>
  · rw [disjoint_insert_right] at hCD
...
lemma exists_splice_of_contract_eq_delete' {C D : Set α} (hfin : C.Finite ∨ D.Finite)
  ...
  wlog hDfin : D.Finite generalizing C D M N with aux
  · rw [or_iff_left hDfin] at hfin
    obtain ⟨P, hPN, hPM⟩ := aux (.inr hfin) disjoint.symm
      (by rwa [← dual_inj, dual_delete_dual, dual_contract_dual, eq_comm]) hfin
    exact ⟨P✶, by rwa [← dual_inj, dual_delete_dual], by rwa [← dual_inj, dual_contract_dual]⟩
  obtain ⟨P, -, rfl, rfl⟩ := exists_splice_of_contract_eq_delete disjoint hDfin con_eq_del
  exact ⟨P, rfl, rfl⟩
