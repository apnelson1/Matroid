...
def matroid_on_univ_of_subspace (U : Submodule 𝔽 (α → 𝔽)) (hU : Module.Finite 𝔽 U) : Matroid α := 
  ...
  ( by 
    ...
    rw [v.indep_iff_projSet_eq_top (I := I), v.projSet_eq_map, Rep.subspaceRep]
    have hUf : (ofFun 𝔽 <| fun a i ↦ (b i).1 a) = U := b.eq_ofFun
    simp_rw [←hUf]
    rfl )

noncomputable def matroid_of_subspace (E : Set α) (U : Submodule 𝔽 (E → 𝔽)) 
    (hU : Module.Finite 𝔽 U) : Matroid α  := 
  matroid_on_univ_of_subspace 
