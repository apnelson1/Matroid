...
/-- `v.subtypeCycleSpace` is the cycle space of `v` as a subspace of `M.E →₀ 𝔽`.-/
noncomputable def Rep.subtypeCycleSpace (v : M.Rep 𝔽 W) : Submodule 𝔽 (M.E →₀ 𝔽) :=
  LinearMap.ker <| Finsupp.linearCombination 𝔽 (fun x : M.E ↦ v x)

/-- `v.cycleSpace` -/
noncomputable def Rep.cycleSpace (v : M.Rep 𝔽 W) : Submodule 𝔽 (α →₀ 𝔽) :=
  v.subtypeCycleSpace.map (Finsupp.lEmbDomain _ (Function.Embedding.subtype M.E))

@[simp]
lemma mem_subtypeCycleSpace_iff (v : M.Rep 𝔽 W) (x : M.E →₀ 𝔽) :
    x ∈ v.subtypeCycleSpace ↔ Finsupp.linearCombination 𝔽 (fun i : M.E ↦ v i) x = 0 := Iff.rfl

lemma mem_cycleSpace_iff (v : M.Rep 𝔽 W) (x : α →₀ 𝔽) :
    x ∈ v.cycleSpace ↔ Finsupp.linearCombination 𝔽 v x = 0 ∧ (x.support : Set α) ⊆ M.E := by
  classical
  simp only [Rep.cycleSpace, Finsupp.lEmbDomain, mem_map, mem_subtypeCycleSpace_iff,
    LinearMap.coe_mk, AddHom.coe_mk, Finsupp.fun_support_eq]
  refine ⟨?_, fun ⟨h, h'⟩ ↦ ?_⟩
  · rintro ⟨c, hc, rfl⟩
    exact ⟨by simpa using hc, by simp [Finsupp.support_subset_iff]⟩
  refine ⟨Finsupp.supportedEquivFinsupp M.E (⟨x, h'⟩ : Finsupp.supported 𝔽 𝔽 M.E), ?_, ?_⟩
  · sorry
...
/-- `subtypeCycleSpace` is the cocycle space as a subspace of `M.E → 𝔽`.-/
noncomputable def Rep.subtypeCocycleSpace (v : M.Rep 𝔽 W) :
    Submodule 𝔽 (M.E → 𝔽) :=
  (dualAnnihilator v.subtypeCycleSpace).map (Finsupp.dualEvalSingle ..)


noncomputable def Rep.cocycleSpace (v : M.Rep 𝔽 W) : Submodule 𝔽 (α → 𝔽) :=
  Submodule.map (ExtendByZero.linearMap 𝔽 Subtype.val) v.subtypeCocycleSpace

lemma foo (v : M.Rep 𝔽 W) (φ : W →ₗ[𝔽] W') (hφ) :
    (v.comp φ hφ).subtypeCycleSpace = v.subtypeCycleSpace := by
  sorry

-- instance [M.Finite] : Finite ↥M.E :=
--   M.ground_finite



  -- Submodule.map (Finsupp.lExtendDomain (M := 𝔽) (R := 𝔽) (P := (· ∈ M.E))) v.subtypeCycleSpace



    -- (LinearMap.ker (Finsupp.linearCombination _ 𝔽 v M.E))

-- def Rep.supportedCocyleSpace [M.Finite] (v : M.Rep 𝔽 W)

-- instance Rep.supported_finiteDimensional [M.Finite] :
