...
lemma rk_def (M : Matroid α) : M.rk = M.r M.E := by
  rw [rk,r,eRk,restrict_ground_eq_self]

@[simp] lemma eRk_toNat_eq_r (M : Matroid α) (X : Set α) : ENat.toNat (M.eRk X) = M.r X :=
  rfl

lemma Base.ncard (hB : M.Base B) : B.ncard = M.rk := by
  rw [rk_def, ← eRk_toNat_eq_r, ncard_def, hB.encard, eRank_def]

lemma rFin.cast_r_eq (hX : M.rFin X) : (M.r X : ℕ∞) = M.eRk X := by
  rw [r, coe_toNat (by rwa [eRk_ne_top_iff])]

lemma cast_r_eq_eRk_of_finite (M : Matroid α) (hX : X.Finite) : (M.r X : ℕ∞) = M.eRk X :=
  (M.rFin_of_finite hX).cast_r_eq

lemma Finset.cast_r_eq (M : Matroid α) (X : Finset α) : (M.r X : ℕ∞) = M.eRk X :=
  cast_r_eq_eRk_of_finite _ (by simp)

@[simp] lemma cast_r_eq (M : Matroid α) [FiniteRk M] (X : Set α) : (M.r X : ℕ∞) = M.eRk X :=
  (M.to_rFin X).cast_r_eq

@[simp] lemma coe_rk_eq (M : Matroid α) [FiniteRk M] : (M.rk : ℕ∞) = M.eRank := by
  rw [eRank_def, rk_def, cast_r_eq]

lemma r_eq_of_eRk_eq (h : M.eRk X = M.eRk Y) : M.r X = M.r Y := by
  rw [r, r, h]

lemma rFin.eRk_eq_eRk_iff (hX : M.rFin X) (hY : M.rFin Y) : M.eRk X = M.eRk Y ↔ M.r X = M.r Y := by
  rw [← hX.cast_r_eq, ← hY.cast_r_eq, Nat.cast_inj]

lemma rFin.eRk_le_eRk_iff (hX : M.rFin X) (hY : M.rFin Y) : M.eRk X ≤ M.eRk Y ↔ M.r X ≤ M.r Y := by
  rw [← hX.cast_r_eq, ← hY.cast_r_eq, Nat.cast_le]

@[simp] lemma eRk_eq_eRk_iff [FiniteRk M] : M.eRk X = M.eRk Y ↔ M.r X = M.r Y :=
  (M.to_rFin X).eRk_eq_eRk_iff (M.to_rFin Y)

@[simp] lemma eRk_le_eRk_iff [FiniteRk M] : M.eRk X ≤ M.eRk Y ↔ M.r X ≤ M.r Y := by
  rw [← cast_r_eq, ← cast_r_eq, Nat.cast_le]

@[simp] lemma eRk_eq_coe_iff [FiniteRk M] {n : ℕ} : M.eRk X = n ↔ M.r X = n := by
  rw [← cast_r_eq, Nat.cast_inj]

@[simp] lemma eRk_le_coe_iff [FiniteRk M] {n : ℕ} : M.eRk X ≤ n ↔ M.r X ≤ n := by
  rw [← cast_r_eq, Nat.cast_le]

@[simp] lemma coe_le_eRk_iff [FiniteRk M] {n : ℕ} : (n : ℕ∞) ≤ M.eRk X ↔ n ≤ M.r X := by
  rw [← cast_r_eq, Nat.cast_le]

lemma rFin.r_le_r_of_eRk_le_eRk (hY : M.rFin Y) (hle : M.eRk X ≤ M.eRk Y) : M.r X ≤ M.r Y := by
  rwa [← rFin.eRk_le_eRk_iff _ hY]; exact hle.trans_lt hY.lt

lemma r_inter_ground (M : Matroid α) (X : Set α) : M.r (X ∩ M.E) = M.r X := by
  rw [← eRk_toNat_eq_r, eRk_inter_ground, eRk_toNat_eq_r]
...
lemma Indep.r_eq_ncard (hI : M.Indep I) : M.r I = I.ncard := by
  rw [← eRk_toNat_eq_r, hI.er, ncard_def]
...
lemma Basis'.card (h : M.Basis' I X) : I.ncard = M.r X := by
  rw [ncard_def, h.encard, ← eRk_toNat_eq_r]
...
lemma Circuit.r_add_one_eq {C : Finset α} (hC : M.Circuit C) : M.r C + 1 = C.card := by
  rw [← Nat.cast_inj (R := ℕ∞), Nat.cast_add, ← encard_coe_eq_coe_finsetCard,
    ← hC.eRk_add_one_eq, cast_r_eq_eRk_of_finite _ (by simp), Nat.cast_one]
...
lemma ext_rank {M N : Matroid α} [FiniteRk M] [FiniteRk N] (hE : M.E = N.E)
  ...
  · rw [indep_iff_eRk_eq_encard_of_finite hI.finite, ← cast_r_eq_eRk_of_finite _ hI.finite, ← h _ hIE,
