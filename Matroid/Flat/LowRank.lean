...
@[reducible] def Point (M : Matroid α) (P : Set α) := M.IsFlat P ∧ M.eRk P = 1

lemma Point.isFlat (hP : M.Point P) : M.IsFlat P :=
  hP.1
...
lemma point_contract_iff (hC : C ⊆ M.E := by aesop_mat) :
  ...
  rw [Point, isFlat_contract_iff, covBy_iff_eRelRk_eq_one, eRelRk_closure_left,
...
lemma Point.eq_or_eq_of_isFlat_of_subset (hP : M.Point P) (hF : M.IsFlat F) (h : F ⊆ P) :
    F = M.closure ∅ ∨ F = P :=
  hP.covBy.eq_or_eq hF hF.loops_subset h

lemma Point.subset_or_inter_eq_loops_of_isFlat (hP : M.Point P) (hF : M.IsFlat F) :
...
-- def IsFlat.covByPointPartition {F : Set α} (hF : M.IsFlat F) :
...
abbrev Line (M : Matroid α) (L : Set α) := M.IsFlat L ∧ M.eRk L = 2

lemma Line.isFlat (hL : M.Line L) : M.IsFlat L :=
  hL.1
...
abbrev Plane (M : Matroid α) (P : Set α) := M.IsFlat P ∧ M.eRk P = 3

lemma Plane.isFlat (hP : M.Plane P) : M.IsFlat P :=
  hP.1
