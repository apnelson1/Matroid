...
private lemma diff_aux_of_exch {base : Set α → Prop} (exch : exchange_property base) 
    {B₁ B₂ : Set α} (hB₁ : base B₁) (hB₂ : base B₂) (hfin : (B₁ \ B₂).Finite) :
    (B₂ \ B₁).Finite ∧ (B₂ \ B₁).ncard = (B₁ \ B₂).ncard := by
  ...
  -- have foo : B₁ \ insert f (B₂ \ {e}) = insert f ((B₁ \ B₂) \ {e})
  -- · rw [←insert_diff_singleton]

  have IH : ncard (B₁ \ insert f (B₂ \ {e})) < ncard (B₁ \ B₂)
  · apply ncard_lt_ncard _ hfin 
    refine' (ssubset_insert (by aesop)).trans_subset (insert_subset.mpr ⟨hf,_⟩)
    rw [diff_subset_iff]
  ...
  -- have IH : ncard (B₁ \ insert f (B₁ \ {e})) < (B₁ \ B₂).ncard
  -- sorry
  have := diff_aux_of_exch exch hB₁ hB'
  sorry
termination_by _ => (B₁ \ B₂).ncard
  ...
  -- have hfin' := hfin
  -- obtain ⟨f, hf, hB'⟩ := exch B₁ B₂ hB₁ hB₂ e he
  -- obtain ⟨hfin, hcard⟩ := diff_aux_of_exch exch (insert f (B₁ \ {e})) B₂ hB' hB₂ _
  -- · sorry
  
  -- · rw [←union_singleton, union_diff_distrib] at hcard
