...
lemma isCover_closure {P : Matroid α → Set α → Prop} (h : T.IsCover X (P M))
    (hP : ∀ F, P M F → P M (M.closure F)) (hXc : M.IsFlat X) :
    (M.closure '' T).IsCover X (P M) := by
  convert h.image M.closure (Q := P M) (fun y _ ↦ hP y)
  rw [subset_antisymm_iff, iUnion₂_subset_iff]
  refine ⟨?_, fun F hFT ↦ by grw [h.subset hFT, hXc.closure]⟩
  grw [← iUnion₂_mono fun F (hF : F ∈ T) ↦ M.inter_ground_subset_closure F, ← iUnion₂_inter,
    ← sUnion_eq_biUnion, h.sUnion_eq, inter_eq_self_of_subset_left hXc.subset_ground]

lemma isCover_rkLE_of_contract (h : T.IsCover Y ((M ／ C).RkLE k)) (hYne : Y.Nonempty) :
    ((· ∪ C) '' T).IsCover (Y ∪ C) (M.RkLE (k + M.eRk C)) := by
  convert h.image (· ∪ C) ?_
  · rw [← biUnion_distrib_union _ (h.nonempty hYne), h.biUnion_eq]
  exact fun X _ hX ↦ hX.union_of_contract

lemma isCover_rkLE_iff_restriction :
    T.IsCover X (M.RkLE k) ↔ T.IsCover (M ↾ X).E ((M ↾ X).RkLE k) :=
  isCover_congr' fun Y hYX ↦ by simp [inter_eq_self_of_subset_left hYX]

lemma isCover_closure_inter (h : T.IsCover X (M.RkLE k)) (hX : X ⊆ M.E := by aesop_mat) :
    ((M.closure · ∩ X) '' T).IsCover X (M.RkLE k) := by
  convert h.image _ fun Y hYX hY ↦ hY.closure.subset inter_subset_left
  grw [← iUnion₂_inter, eq_comm, inter_eq_right,
...
lemma IsFlat.isCover_closure_of_isCover (hF : M.IsFlat F) (h : T.IsCover F (M.RkLE k)) :
  ...
  refine EqOn.image_eq fun X hX ↦ ?_
  grw [eq_comm, inter_eq_left, hF.closure_subset_of_subset (h.subset hX)]

lemma isCover_rkLE_nontrivial (hcov : T.IsCover X (M.RkLE k)) (hk : k < M.eRk X) :
    T.Nontrivial :=
  hcov.nontrivial_of_prop_empty (by simpa [RkLE]) <| by simp

lemma setOf_singleton_isCover_rkLE (M : Matroid α) (X : Set α) :
...
lemma coverNumber_rkLE_subset (M : Matroid α) (hXY : X ⊆ Y) (k : ℕ∞) :
    X.coverNumber (M.RkLE k) ≤ Y.coverNumber (M.RkLE k) :=
  coverNumber_subset_of_antitone (by simp) hXY

/-- The set of points of a positive-rank matroid `M` is a cover of `M` -/
lemma setOf_point_isCover_rkLE (M : Matroid α) [M.RankPos] :
  ...
  · refine subset_antisymm (fun e he ↦ ?_) (iUnion₂_subset (by simp [closure_subset_ground]))
    ...
    exact ⟨e, he', M.mem_closure_self e⟩
  simp

lemma setOf_point_restrict_isCover_rkLE (M : Matroid α) (X : Set α) (hX : M.eRk X ≠ 0) :
