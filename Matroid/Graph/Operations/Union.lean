...
lemma isLink_le_iUnion (hG : Pairwise (Graph.Compatible on G)) (i : ι) :
  ...
  refine fun hxy ↦ ⟨x, ?_, y, ?_, ?_⟩ <;> simp only [iSup_rel,
...
lemma iUnion_isLink_of_dup_agree (hG : Pairwise (Graph.Compatible on G))
  ...
  refine ⟨fun ⟨a, hxa, b, hlba, hby⟩ => ?_, fun ⟨i, hlxy⟩ => isLink_le_iUnion hG i hlxy⟩
  obtain ⟨i, hlba⟩ := by simpa using hlba
  rw [dup_of_pairwise_dup_agree_of_mem hG' hlba.left_mem] at hby
  rw [comm_of (⇑(⨆ i, (G i).Dup)), dup_of_pairwise_dup_agree_of_mem hG' hlba.right_mem] at hxa
...
protected lemma le_iUnion (hG : Pairwise (Graph.Compatible on G))
  ...
  le_of (subset_iSup_of_agree hG' i) (fun _ _ _ ↦ isLink_le_iUnion hG i)
...
protected lemma iUnion_le_iff (hG : Pairwise (Graph.Compatible on G))
    (hG' : Pairwise (Dup_agree on G)) : Graph.iUnion G hG ≤ H ↔ ∀ i, G i ≤ H := by
...
protected lemma iUnion_le_iff_of_nodup (hG : Pairwise (Graph.Compatible on G))
    [hN : ∀ i, Nodup (G i)] : Graph.iUnion G hG ≤ H ↔ ∀ i, G i ≤ H :=
...
@[simp↓]
lemma iUnion_inc_iff_of_agree (hG : Pairwise (Graph.Compatible on G))
    (hG' : Pairwise (Dup_agree on G)) :
...
@[simp]
lemma iUnion_isLoopAt_iff_of_agree (hG : Pairwise (Graph.Compatible on G))
    (hG' : Pairwise (Dup_agree on G)) :
...
lemma iUnion_map_le_iUnion_of_nodup (hG : Pairwise (Graph.Compatible on G)) (f : ι' → ι)
    [hN : ∀ i, Nodup (G i)] :
    (Graph.iUnion (G ∘ f) (pairwise_compatible_comp hG f)) ≤ Graph.iUnion G hG :=
...
--     (hG' : Pairwise (Dup_agree on G)) (hH' : Pairwise (Dup_agree on H)) :
--     Graph.iUnion G hGH.sum_left ≤ Graph.iUnion (Sum.elim G H) hGH := by
--   rw [Graph.iUnion_le_iff _ hG']
--   refine fun i ↦ le_trans (by simp) (Graph.le_iUnion hGH ?_ (Sum.inl i))
--   sorry
...
--   iUnion_left_le_iUnion_sum hGH pairwise_dup_agree_of_nodup pairwise_dup_agree_of_nodup
