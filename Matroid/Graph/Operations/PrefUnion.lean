...
protected lemma iInter_comp_le {ι' : Type _} [Nonempty ι] [Nonempty ι'] {f : ι' → ι}
    {G : ι → Graph α β} (hG : Pairwise (Compatible on G)) :
    Graph.iInter G hG ≤ Graph.iInter (fun i ↦ G (f i)) (by
    change Pairwise ((Compatible on G) on f)
    rw [Pairwise.eq_true_of_refl hG]
    tauto) := by
...
protected lemma iInter_comp_eq_of_surj {ι' : Type _} [Nonempty ι] [Nonempty ι'] {f : ι' → ι}
    {G : ι → Graph α β} (hG : Pairwise (Compatible on G)) (hf : Function.Surjective f) :
    Graph.iInter G hG = Graph.iInter (fun i ↦ G (f i)) (by
    change Pairwise ((Compatible on G) on f)
    rw [Pairwise.eq_true_of_refl hG]
    tauto) := by
  refine le_antisymm (Graph.iInter_comp_le hG) ?_
  rw [Graph.le_iInter_iff]
  rintro i
  obtain ⟨i', rfl⟩ := hf i
  exact Graph.iInter_le (_ : Pairwise (Compatible on (G ∘ f))) i'
...
protected lemma sInter_anti {s t : Set (Graph α β)} (hs : s.Pairwise Compatible) (hne : s.Nonempty)
    (ht : t.Pairwise Compatible) (hne' : t.Nonempty) (hle : s ⊆ t) :
    Graph.sInter t ht hne' ≤ Graph.sInter s hs hne := by
  rw [Graph.le_sInter_iff hs hne]
...
protected lemma sInter_insert_eq {s : Set (Graph α β)} (hs : (insert G s).Pairwise Compatible)
    (hne : s.Nonempty) [DecidablePred (· ∈ s)] : Graph.sInter (insert G s) hs (by simp) =
    Graph.iInter ((fun G : (insert G s : Set _) ↦ G.1) ∘ (Equiv.insert_option G (sorry : G ∉ s)))
    sorry := by
  unfold Graph.sInter
  apply Graph.iInter_comp_eq_of_surj
  exact Equiv.surjective (Equiv.insert_option G sorry)
