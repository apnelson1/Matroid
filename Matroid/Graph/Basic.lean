...
@[simp]
lemma vertexPartition_parts : P(G).parts = V(G) := by
  rw [← G.vertexSet_eq_parts]

@[simp]
lemma mem_vertexPartition_iff : x ∈ P(G) ↔ x ∈ V(G) := by
  rw [← vertexPartition_parts]
  exact mem_parts

@[simp]
lemma vertexPartition_subset_iff {H : Graph α β} : P(G) ⊆ P(H) ↔ V(G) ⊆ V(H) := by
...
instance : IsSymm _ (G.IsLink e) where
  symm _ _ h := h.symm

lemma IsLink.left_mem (h : G.IsLink e x y) : x ∈ V(G) :=
  G.left_mem_of_isLink h

lemma domain_IsLink_subset_vertexSet (G : Graph α β) (e : β) :
    Relation.domain (G.IsLink e) ⊆ V(G) := by
  rintro x ⟨y, hxy⟩
  exact hxy.left_mem
...
lemma codomain_IsLink_subset_vertexSet (G : Graph α β) (e : β) :
    Relation.codomain (G.IsLink e) ⊆ V(G) := by
  rintro y ⟨x, hxy⟩
  exact hxy.right_mem
...
lemma IsLink.eq_and_eq_or_eq_and_eq (h : G.IsLink e x y) (h' : G.IsLink e x' y') :
    (x = x' ∧ y = y') ∨ (x = y' ∧ y = x') := by
  obtain rfl | rfl := h.left_eq_or_eq h'
  · simp [h.right_unique h']
  simp [h'.symm.right_unique h]

lemma IsLink.isLink_iff (h : G.IsLink e x y) :
...
lemma IsLink.isLink_iff_sym2_eq (h : G.IsLink e x y) :
...
lemma isLink_eq_isLink_iff_exists_isLink_of_mem_edgeSet {H : Graph α β} (heG : e ∈ E(G)) :
...
lemma mk_eq_self (G : Graph α β) {E : Set β} (hE : ∀ e, e ∈ E ↔ ∃ x y, G.IsLink e x y) :
    Graph.mk P(G) V(G) G.vertexSet_eq_parts G.IsLink E
...
lemma vertexPartition_ext {G₁ G₂ : Graph α β} (hV : P(G₁) = P(G₂))
  ...
  convert rfl using 2
  · exact hV.symm
  · simpa using Partition.ext_iff_parts.mp hV.symm
  · simp [funext_iff, h]
...
protected lemma ext {G₁ G₂ : Graph α β} (hV : V(G₁) = V(G₂))
    (h : ∀ e x y, G₁.IsLink e x y ↔ G₂.IsLink e x y) : G₁ = G₂ := by
  refine vertexPartition_ext (Partition.ext_parts ?_) h
  rwa [G₁.vertexSet_eq_parts, G₂.vertexSet_eq_parts]
