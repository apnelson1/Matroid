...
lemma Compatible.union_isLink_iff (h : Compatible G H) :
  ...
  by_cases heG : e ∈ E(G)
  · rw [Graph.union_isLink_iff, iff_true_intro heG, not_true, false_and, or_false, iff_self_or]
    exact fun he ↦ by rwa [h ⟨heG, he.edge_mem⟩]
...
edges are precisely those on which `G` and `H` agree, and the edge set is a subset of `E(G) ∩ E(H)`,
...
lemma Compatible.union_eq_iUnion (h : G.Compatible H) :
  ...
  simp only [le_antisymm_iff, h.union_le_iff, Graph.iUnion_le_iff, Bool.forall_bool, cond_false,
    h.right_le_union, cond_true, left_le_union, and_self, and_true]
  refine ⟨Graph.le_iUnion h' true, Graph.le_iUnion h' false⟩
...
lemma Compatible.union_eq_sUnion (h : G.Compatible H) :
  ...
  le_antisymm_iff.2 <| by simp +contextual [h.right_le_union, h.union_le_iff]
