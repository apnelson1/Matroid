...
lemma IsAugmenter.matched_vertexSet_eq (hP : G.IsAugmenter M P) :
  ...
  ext x
  refine ⟨fun hx ↦ ⟨by grind, ?_⟩, by grind⟩
  by_contra! bad
...
lemma IsAugmenter.maxDegreeLE_two (hP : G.IsAugmenter M P) : P.MaxDegreeLE 2 := by
  rw [maxDegreeLE_iff']
  intro x hx
  obtain (h | h) := hP.eDegree_eq_one_or_two hx <;> (enat_to_nat!; omega)
...
lemma IsAugmenter.diff_matching_isMatching [P.Loopless] (hP : G.IsAugmenter M P) :
  ...
  disjoint e f he hf hne := by
    ...
    have h_incEdges : E(P, x) = {e, f} := by
      ...
      simp only [← ncard_def, Nat.cast_ofNat, ENat.toNat_ofNat] at deg
    ...
    simp only [h_incEdges, mem_insert_iff, mem_singleton_iff] at he'
    obtain (rfl | rfl) := he' <;> [exact he.2 ‹_›; exact hf.2 ‹_›]
...
lemma IsAugmenter.vertexSet_disjoint (hM : G.IsMatching M) (hP : G.IsAugmenter M P) :
  ...
  have hxGM : x ∈ V(G, M) := by
    simp only [mem_incVertexSet_iff, mem_sdiff] at hxGM'
    obtain ⟨e, he⟩ := hxGM'
    refine ⟨e, he.1.1, he.2⟩
  obtain (h | h) := hP.eDegree_eq_one_or_two hxP
  · exact hP.no_match_leaf ‹_› hxGM
  have M_diff_matching : G.IsMatching (M \ E(P)) := hM.anti_right sdiff_subset
  have P_matching : P.IsMatching (E(P) ∩ M) := hM.anti_left' hP.le
  have hxPM : x ∈ V(P, E(P) ∩ M) := by
    have ⟨e, he⟩ := hP.match_nonleaf h
    exact ⟨e, ⟨he.2.edge_mem, he.1⟩, he.2⟩
  obtain ⟨e, he, e_unique⟩ := hM.existsUnique_covering_edge hxGM
  obtain ⟨e', he', -⟩ := M_diff_matching.existsUnique_covering_edge hxGM'
  obtain ⟨f, hf, -⟩ := P_matching.existsUnique_covering_edge hxPM
  simp only [and_imp, mem_sdiff, mem_inter_iff] at e_unique he he' hf
  obtain rfl : e' = f := by rw [e_unique _ he'.1.1 he'.2, e_unique _ hf.1.2 (hf.2.of_le hP.le)]
...
lemma IsAugmenter.symmDiff_matching_isMatching [P.Loopless] (hM : G.IsMatching M)
  ...
  have disj : P.StronglyDisjoint (Subgraph.ofEdge G (M \ E(P))) := by
    refine ⟨?_, ?_⟩
    · simp only [Subgraph.ofEdge_vertexSet]
      exact hP.vertexSet_disjoint hM
    rw [disjoint_left]
    simp only [Subgraph.ofEdge_edgeSet, mem_inter_iff, mem_sdiff, not_and, not_not]
    grind only
  ...
  have G'_matching : G'.val.IsMatching (M \ E(P)) := by
    have matching : G.IsMatching (M \ E(P)) :=
      hM.anti_right sdiff_subset
    refine matching.anti_left G'.le ?_
    simp only [Subgraph.ofEdge_edgeSet, subset_inter_iff, sdiff_subset_iff, subset_refl, and_true,
      G']
    grw [← subset_union_right, hM.subset]
  have := P_matching.union G'_matching disj
  rw [symmDiff_comm, Set.symmDiff_def]
  exact this.mono_left (Graph.union_le hP.le G'.le)
...
lemma IsAugmenter.diff_matching_vertexSet [P.Loopless] (hM : G.IsMatching M)
  ...
  refine eq_of_subset_of_subset (incVertexSet_subset _ _) fun x hxP ↦ ?_
  ...
  · have h : P.eDegree x = 1 := deg
    ...
    have he_notMem_M : e ∉ M := by
      exact fun heM ↦ hP.no_match_leaf deg (⟨e, heM, he.of_le hP.le⟩)
    exact ⟨e, ⟨he.edge_mem, he_notMem_M⟩, he⟩
  -- degree 2
  have ⟨e, he⟩ := hP.match_nonleaf deg
  rw [eDegree_eq_encard_inc] at deg
  symm at deg
  apply le_of_eq at deg
  rw [two_le_encard_iff_nontrivial] at deg
  have ⟨f, hf⟩ := deg.exists_ne e
  refine ⟨f, ⟨hf.1.edge_mem, fun hfM ↦ ?_⟩, hf.1⟩
  have := hM.disjoint hfM he.1 hf.2
  rw [disjoint_left] at this
  exact this (hf.1.of_le hP.le) (he.2.of_le hP.le)

-- TODO: move
lemma incVertexSet_mono_right {F F' : Set β} (G : Graph α β) (hsub : F ⊆ F') : V(G, F) ⊆ V(G, F') :=
  fun _ ⟨e, heF, hex⟩ ↦ ⟨e, hsub heF, hex⟩
...
-- be finite).
lemma IsAugmenter.matching_encard_add_one_le_diff_matching_encard [P.Loopless] (hM : G.IsMatching M)
    (hP : G.IsAugmenter M P) : (E(P) ∩ M).encard + 1 ≤ (E(P) \ M).encard := by
  ...
  have M_matching : P.IsMatching (E(P) ∩ M) := hM.anti_left' hP.le
  ...
  exact ⟨x, hx.mem, fun ⟨e, hePM, hex⟩ ↦ hP.no_match_leaf hx.eDegree ⟨e, hePM.2, hex.of_le hP.le⟩⟩
...
lemma IsMaxMatching.not_isAugmenter [G.EdgeFinite] (hM : G.IsMaxMatching M) (P : Graph α β)
    [P.Loopless] : ¬ G.IsAugmenter M P := by
  intro hP
...
@[simp]
lemma Subgraph.ofEdge_incEdges_eq (F : Set β) : E(Subgraph.ofEdge G F, x) = E(G, x) ∩ F := by
  ext e
  simp only [IncEdges, ofEdge_inc_iff, mem_ofPred_eq, mem_inter_iff]
  rw [and_comm]
...
private lemma symmDiff_edge_mem_or_mem [G.Loopless] (hle : H ≤ Subgraph.ofEdge G (M ∆ M')) (e : β)
    (he : e ∈ E(H)) : e ∈ M \ M' ∨ e ∈ M' \ M := by
  have := edgeSet_mono hle
  simp only [Subgraph.ofEdge_edgeSet, subset_inter_iff] at this
  apply this.2 at he
  rw [Set.symmDiff_def] at he
  exact he

private lemma symmDiff_matching_edge_at [G.Loopless] (hM : G.IsMatching M)
    (hle : H ≤ Subgraph.ofEdge G (M ∆ M')) (hex : e ∈ E(H, x)) (hfx : f ∈ E(H, x)) (hne : e ≠ f)
    (heM : e ∈ M) : f ∈ M' := by
  by_contra! hfM
  replace hfM : f ∈ M := by
...
private lemma symmDiff_matching_internal_vx [G.Loopless] (hM : G.IsMatching M)
    (hM' : G.IsMatching M') (hle : H ≤ Subgraph.ofEdge G (M ∆ M')) (hdeg : H.eDegree x = 2) :
    x ∈ V(H, M) ∧ x ∈ V(H, M') := by
  have H_le_G : H ≤ G := hle.trans <| Subgraph.le _
  ...
  · refine aux f e hne.symm (by rw [heq, pair_comm]) ?_
    have he : e ∈ E(H, x) := by grind only [= mem_insert_iff]
    have hf : f ∈ E(H, x) := by grind only [= mem_insert_iff, = mem_singleton_iff]
    have heM' : e ∈ M' := by grind only [= mem_sdiff, H.incEdges_subset x he]
...
private
lemma symmDiff_leaf_vx [G.Loopless] (hle : H ≤ Subgraph.ofEdge G (M ∆ M'))
    (hdeg : H.eDegree x = 1) : x ∈ V(H, M) \ V(H, M') ∨ x ∈ V(H, M') \ V(H, M) := by
  ...
  · rw [symmDiff_comm] at hle
    ...
    -- `grind only [IsPendant.edge_mem he, = mem_sdiff]`.
    grind only [→ IsPendant.edge_mem, = mem_sdiff]
  refine Or.inl ⟨⟨e, heM, he.inc⟩, fun ⟨f, hf⟩ ↦ ?_⟩
  have := he.edge_unique hf.2
  have := symmDiff_edge_mem_or_mem hle
  grind only [→ Inc.edge_mem, = mem_sdiff]

private lemma symmDiff_matching_cycle_edge_encard [G.Loopless] (hM : G.IsMatching M)
    (hM' : G.IsMatching M') (hle : H ≤ Subgraph.ofEdge G (M ∆ M')) (h_cycle : H.IsCycle) :
...
lemma exists_isAugmenter_of_matching_encard_lt [G.Loopless] [G.EdgeFinite] (hM : G.IsMatching M)
    (hM' : G.IsMatching M') (hlt : M.encard < M'.encard) : ∃ P, G.IsAugmenter M P := by
  let P := Subgraph.ofEdge G (M ∆ M')
  have P_loopless : P.val.Loopless := ‹G.Loopless›.mono P.2
  have P_degPos : P.val.DegreePos := by
    rw [degreePos_iff']
    intro x ⟨e, he⟩
    grw [← Order.one_le_iff_ne_zero, ← encard_inc_le_eDegree, one_le_encard_iff_nonempty,
  ...
  have P_maxDegreeLE : P.val.MaxDegreeLE 2 := by
    ...
    all_goals
      ...
      exact IsMatching.anti_right ‹_› sdiff_subset
  ...
  obtain ⟨P', hP'P, hP'_encard⟩ :
    ...
    replace h_encard_le_encard : (M' \ M).encard ≤ (M \ M').encard := by
      simp only [Subgraph.ofEdge_edgeSet, P] at h_encard_le_encard
      ...
      simp only [union_inter_distrib_right, sdiff_inter_self, empty_union,
        union_empty] at h_encard_le_encard
      rwa [inter_eq_left.mpr (by grind), inter_eq_left.mpr (by grind)] at h_encard_le_encard
    rw [← encard_le_encard_iff_encard_sdiff_le_encard_sdiff] at h_encard_le_encard
    · rw [← not_le] at hlt
  ...
  refine ⟨P', hP'P.le.trans P.2, P'_deg, ⟨hP'_path.first, hP'_path.isLeaf_first ?_⟩,
    fun x hx ↦ symmDiff_matching_internal_vx hM hM' hP'P.le hx |>.1, ?_⟩
  · -- P' is a path, so we must have a leaf?
    ...
    intro bad
    have : (E(P') ∩ M').encard ≤ E(P').encard := encard_le_encard inter_subset_left
    enat_to_nat! <;> omega
  ...
  have P'_leaf : {x | P'.eDegree x = 1} = V(P', M) \ V(P', M') ∪ V(P', M') \ V(P', M) := by
    ...
    refine ⟨symmDiff_leaf_vx hP'P.le, fun hx ↦ ?_⟩
    have hxP' : x ∈ V(P') := by grind only [!incVertexSet_subset, = mem_sdiff, = subset_def]
    by_contra! hdeg
  ...
  rw [encard_lt_encard_iff_encard_sdiff_lt_encard_sdiff
  ...
  have hexP' : P'.Inc e x := by
    ...
    grind only [!incVertexSet_subset, = mem_sdiff, = subset_def]
...
lemma berge [G.Loopless] [G.EdgeFinite] (hM : G.IsMatching M) :
  ...
  refine ⟨fun h_notMax ↦ ?_, fun ⟨P, hP⟩ hM_max ↦ ?_⟩
  · simp only [isMaxMatching_iff, not_and, not_forall, not_le] at h_notMax
    obtain ⟨M', hM', hlt⟩ := h_notMax hM
