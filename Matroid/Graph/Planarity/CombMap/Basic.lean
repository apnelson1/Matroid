...
lemma sigma_finiteOrbit (M : CombinatorialMap G D) (hM : M.toDartStructure.LocallyFinite) :
    M.σ.finiteOrbit := by
...
noncomputable def restrict (M : CombinatorialMap G D) (hM : M.toDartStructure.LocallyFinite)
    (F : Set E) : CombinatorialMap (G ↾ F) D where
  toDartStructure := M.toDartStructure.restrict F
  σ := by
    classical
    exact M.σ.keep (M.fₑ.preimage F) (M.sigma_finiteOrbit hM)
  sigma_id d hd := by
    classical
    ...
    exact Equiv.keep_apply_of_not_mem (M.sigma_finiteOrbit hM) hkeep
  transtive_preimage v := by
    classical
    rw [DartStructure.fᵥ_restrict, PFun.preimage_restrict]
    exact (M.transtive_preimage v).keep (M.sigma_finiteOrbit hM) (M.fₑ.preimage F)

noncomputable def induce (M : CombinatorialMap G D) (hM : M.toDartStructure.LocallyFinite)
    (X : Set V) : CombinatorialMap G[X] D where
  toDartStructure := M.toDartStructure.induce X
  σ := by
    classical
    exact M.σ.keep (M.fₑ.preimage E(G[X])) (M.sigma_finiteOrbit hM)
  sigma_id d hd := by
    classical
  ...
  transtive_preimage v := by
    ...
    exact (M.transtive_preimage v).keep (M.sigma_finiteOrbit hM) (M.fₑ.preimage E(G[X]))

noncomputable def deleteVerts (M : CombinatorialMap G D) (hM : M.toDartStructure.LocallyFinite)
    (X : Set V) : CombinatorialMap (G - X) D :=
  (M.induce hM (V(G) \ X)).copy (G.deleteVerts_def X)

noncomputable def of_le (h : H ≤ G) (M : CombinatorialMap G D)
    (hM : M.toDartStructure.LocallyFinite) : CombinatorialMap H D where
  toDartStructure := M.toDartStructure.of_le h
  σ := by
    classical
    exact M.σ.keep (M.fₑ.preimage E(H)) (M.sigma_finiteOrbit hM)
  sigma_id d hd := by
    classical
...
noncomputable def deleteEdges (M : CombinatorialMap G D) (hM : M.toDartStructure.LocallyFinite)
  ...
  σ := by
    classical
    exact M.σ.skip (M.fₑ.preimage F) (M.sigma_finiteOrbit hM)
  sigma_id d hd := by
    classical
    by_cases hdel : d ∈ M.fₑ.preimage F
    · simpa using Equiv.skip_apply_of_mem (M.sigma_finiteOrbit hM) hdel
    have hdom : d ∉ M.fₑ.Dom := by
      rw [DartStructure.dom_fₑ_deleteEdges] at hd
      exact by_contra fun hdom ↦ hd ⟨not_not.mp hdom, hdel⟩
    exact skip_apply_of_fixed (M.sigma_finiteOrbit hM) (M.sigma_id hdom)
  transtive_preimage v := by
    rw [M.preimage_fᵥ_deleteEdges]
    classical
    exact (M.transtive_preimage v).skip (M.sigma_finiteOrbit hM) (M.fₑ.preimage F)
