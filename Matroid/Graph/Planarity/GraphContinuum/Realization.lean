...
@[simp]
lemma Sigma.image_mk_isOpen {ι : Type*} {α : ι → Type*} [∀ i, TopologicalSpace (α i)] (i : ι)
    (X : Set (α i)) : IsOpen (Sigma.mk i '' X) ↔ IsOpen X :=
  (IsOpenEmbedding.isOpen_iff_image_isOpen (IsOpenEmbedding.sigmaMk)).symm

@[simp]
lemma Sum.preimage_image_mk {α β : Type*} (X : Set β) :
...
-- @[simp] lemma vecEmpty_norm_eq_zero : ‖(![] : Fin 0 → ℝ)‖ = 0 := by simp [Pi.norm_def]
...
lemma Eg.isLink (e : E(G)) : G.IsLink e (Eg.src e) (Eg.tgt e) := G.isLink_src_tgt e.prop

@[grind →]
lemma IsNonloopAt.src_ne_tgt (e : E(G)) {x : V(G)} (h : G.IsNonloopAt e x) :
    Eg.src e ≠ Eg.tgt e := by
  obtain ⟨hne, hl⟩ := h.choose_spec
  grind [(Eg.isLink e).eq_and_eq_or_eq_and_eq hl]

/-- Disjoint union of the discrete 0-skeleton and one copy of `EdgeDisk` per edge. -/
abbrev PreRealization (G : Graph α β) := G.Vtx ⊕ Σ (_ : G.Eg), I
...
private theorem glueRel_aux_inr_boundary {e' : E(G)} (h : glueRel_aux G x (Sum.inr ⟨e', t'⟩)) :
    t' = 0 ∨ t' = 1 := by
...
instance glueRel (G : Graph α β) : Setoid (PreRealization G) := EqvGen.setoid (glueRel_aux G)
...
@[simp↓]
theorem glueRel_inr_inr_iff' (e : E(G)) (he : G.IsNonloopAt e (Eg.src e)) (t₁ t₂ : I) :
    G.glueRel (Sum.inr ⟨e, t₁⟩) (Sum.inr ⟨e, t₂⟩) ↔ t₁ = t₂ := by
  simp only [glueRel_inr_inr_iff, true_and, glueRel_inl_iff_glueRel_aux, glueRel_aux_inr_iff,
...
theorem vertexMk_src_eq_edgeMk_zero (e : E(G)) : vertexMk (Eg.src e) = edgeMk e 0 :=
  Quotient.sound (by simp)

theorem vertexMk_tgt_eq_edgeMk_one (e : E(G)) : vertexMk (Eg.tgt e) = edgeMk e 1 :=
  Quotient.sound (by simp)
...
lemma edgeMk_Ioo_disjoint_iff_ne (e₁ e₂ : E(G)) :
  ...
  have := Quotient.exact heq
  simp only [glueRel_inr_inr_iff, glueRel_inl_iff_glueRel_aux, glueRel_aux_inr_iff] at this
  grind
...
private lemma edgeMk_injective (e : E(G)) (he : G.IsNonloopAt e (Eg.src e)) :
  ...
  <;> have hval : G.src e.prop = G.tgt e.prop := congrArg Subtype.val (by simpa [eq_comm] using h1)
  <;> have hlink := hval ▸ G.isLink_src_tgt e.prop <;> exact absurd hlink (he.not_isLoopAt _)

private lemma edgeMk_preimage_image (e : G.Eg) {X : Set I} (h0X : 0 ∉ X) (h1X : 1 ∉ X) :
  ...
  | inr ⟨e', t'⟩ =>
    simp only [glueRel_inr_inr_iff, glueRel_inl_iff_glueRel_aux, glueRel_aux_inr_iff, inl.injEq,
      exists_eq_left'] at h_eq
    grind

theorem isOpen_edgeMk_image (e : G.Eg) {X : Set I} (h0X : 0 ∉ X) (h1X : 1 ∉ X) :
    IsOpen (edgeMk e '' X) ↔ IsOpen X := by
  change IsOpen (Quotient.mk' ⁻¹' (edgeMk e '' X)) ↔ _
  rw [edgeMk_preimage_image e h0X h1X, isOpen_sum_iff]
  simp

private lemma edgeMk_preimage_aux_eq_preimage_mk' (e : G.Eg) (C : Set I) :
...
private lemma inr_sigma_mk_preimage_edgeMk_preimage_aux (e e' : G.Eg) (C : Set I) :
...
private lemma isClosed_inr_sigma_mk_preimage_edgeMk_preimage_aux_of_ne {e' : G.Eg} (hne : e' ≠ e)
    (C : Set I) : IsClosed {x | ∃ t ∈ C, G.glueRel (Sum.inr ⟨e', x⟩) (Sum.inr ⟨e, t⟩)} := by
  have hF_sub : {x | ∃ t ∈ C, G.glueRel (Sum.inr ⟨e', x⟩) (Sum.inr ⟨e, t⟩) } ⊆ {0, 1} := by
    simp only [glueRel_inr_inr_iff, glueRel_inl_iff_glueRel_aux, glueRel_aux_inr_iff]
    grind
  exact (Finite.subset (by simp) hF_sub).isClosed

private lemma inr_inr_same_edge_set_eq_of_isNonloopAt (e : G.Eg) (he : G.IsNonloopAt e (Eg.src e))
    (C : Set I) : {x | ∃ t ∈ C, G.glueRel (Sum.inr ⟨e, x⟩) (Sum.inr ⟨e, t⟩)} = C := by
  simp [glueRel_inr_inr_iff' e he]

private lemma inr_inr_same_edge_set_subset_union_endpoints (e : G.Eg) (C : Set I) :
    {x | ∃ t ∈ C, G.glueRel (Sum.inr ⟨e, x⟩) (Sum.inr ⟨e, t⟩)} ⊆ C ∪ {0, 1} := by
  rintro t ht
  simp only [glueRel_inr_inr_iff, glueRel_inl_iff_glueRel_aux, glueRel_aux_inr_iff] at ht
  grind

private lemma subset_inr_inr_same_edge_set (e : G.Eg) (C : Set I) :
    C ⊆ {x | ∃ t ∈ C, G.glueRel (Sum.inr ⟨e, x⟩) (Sum.inr ⟨e, t⟩)} := by
  rintro t ht
  simp only [glueRel_inr_inr_iff, glueRel_inl_iff_glueRel_aux, glueRel_aux_inr_iff]
  grind

private lemma isClosed_inr_sigma_mk_preimage_edgeMk_preimage_aux_of_eq (e : G.Eg) {C : Set I}
...
private lemma isClosed_inr_preimage_edgeMk_preimage_aux (e₀ : G.Eg) {C : Set I} (hC : IsClosed C) :
...
@[simp]
theorem isClosedEmbedding_edgeMk (e : E(G)) (he : G.IsNonloopAt e (Eg.src e)) :
...
@[simps]
def partialEquiv_vertexMk (v : Vtx G) : PartialEquiv (Fin 0 → ℝ) G.Realization where
  toFun x := vertexMk v
  invFun v := 0
...
@[simps]
noncomputable def partialEquiv_edgeMk (e' : E(G)) : PartialEquiv (Fin 1 → ℝ) G.Realization where
...
noncomputable def Realization.map (G : Graph α β) :
  ...
  | 0, ⟨v⟩ => partialEquiv_vertexMk v
  | 1, ⟨e⟩ => partialEquiv_edgeMk e
  | _ + 2, ⟨i⟩ => Empty.elim i

lemma continuous_partialEquiv_edgeMk (e : G.Eg) : Continuous (partialEquiv_edgeMk e) :=
...
lemma image_map_closedBall (e : G.Eg) :
...
noncomputable instance : Topology.CWComplex (univ : Set G.Realization) where
  ...
  | 1, ⟨e⟩ => (continuous_partialEquiv_edgeMk ..).continuousOn
  | _ + 2, ⟨i⟩ => Empty.elim i
  continuousOn_symm n i := match n, i with
  | 0, ⟨_⟩ => continuous_const.continuousOn
  | 1, ⟨e⟩ => by
    change ContinuousOn (partialEquiv_edgeMk e).invFun (edgeMk e '' Ioo (0 : I) (1 : I))
    ...
    simp only [h_mem, ↓reduceDIte, comp_apply, partialEquiv_edgeMk, f_pre,
  ...
  pairwiseDisjoint' := by
    ...
    have he : ∀ e : E(G), _ '' (Metric.ball 0 1 : Set <| Fin 1 → ℝ) = edgeMk e '' Ioo 0 1 :=
      (partialEquiv_edgeMk · |>.image_source_eq_target)
    have hv : ∀ v : V(G), _ '' (Metric.ball 0 1 : Set <| Fin 0 → ℝ) = {vertexMk v} :=
      (partialEquiv_vertexMk · |>.image_source_eq_target)
    match n₁, i₁, n₂, i₂ with
    | 0, ⟨v₁⟩, 0, ⟨v₂⟩ =>
  ...
  mapsTo' n i :=
    ...
    | 1, ⟨e⟩ => by
      ...
      simp only [mem_sphere_iff_norm, sub_zero, norm_eq_one_iff_fin_1, Fin.isValue,
        ...
        iUnion_iUnion_eq_left, Matrix.zero_empty, partialEquiv_edgeMk_apply, mem_iUnion, mem_image,
        Metric.mem_closedBall, dist_eq_zero_of_subsingleton, true_and, Fin.exists_fin_zero_pi,
        exists_prop] at hx ⊢
      obtain hx | hx := hx
      · exact ⟨⟨Eg.tgt e⟩, by simp [J, hx, vertexMk_tgt_eq_edgeMk_one]⟩
      · exact ⟨⟨Eg.src e⟩, by simp [J, hx, vertexMk_src_eq_edgeMk_zero]⟩
    | n + 2, ⟨i⟩ => Empty.elim i
  closed' A _ h := by
