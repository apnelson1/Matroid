...
lemma dup_of_le_of_mem (hdup : G.Dup x y) (hle : H ≤ G) (hx : x ∈ V(H)) : H.Dup x y := by
  
  rw [Partition.rel_iff_exists] at hdup ⊢
  obtain ⟨S, hS, hxS, hyS⟩ := hdup
  use S, ?_
...
lemma LabelUnique.of_le (hle : H ≤ G) [G.Nodup] : H.LabelUnique where
