...
lemma Flat.closure (hF : M.Flat F) : M.closure F = F := by
  rw [M.closure_eq_sInter F]
  simp [subset_antisymm_iff, sInter_subset_of_mem (by simpa)]
...
lemma Basis'.closure_eq_closure (h : M.Basis' I X) : M.closure I = (M.closure X) ∪ (X \ I) := by
  ...
  rw [h.closure_eq_closure, closure_inter_ground]
...
lemma Base.closure_of_superset (hB : M.Base B) (hBX : B ⊆ X) : M.closure X = M.E :=
  subset_antisymm (M.closure_subset_ground _) (by rw [← hB.closure_eq]; exact M.closure_subset_closure hBX)
...
lemma basis_iff_basis_closure_of_subset' (hIX : I ⊆ X) : M.Basis I X ↔ X ⊆ M.E ∧ M.Basis I (M.closure X) :=
  ⟨fun h ↦ ⟨h.subset_ground, h.basis_closure_right⟩, fun h ↦ h.2.basis_subset hIX (M.subset_closure X h.1)⟩
...
lemma Basis.basis_of_closure_eq_closure (hI : M.Basis I X) (hY : I ⊆ Y) (h : M.closure X = M.closure Y)
...
lemma mem_closure_insert (he : e ∉ M.closure X) (hef : e ∈ M.closure (insert f X)) : f ∈ M.closure (insert e X) := by
