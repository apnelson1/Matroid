...
theorem parallel'_self (h : e ∈ M.E) : M.Parallel' e e :=
    ⟨h,h,rfl⟩
...
theorem Parallel'.eq_of_indep (h : M.Parallel' e f) (hef : M.Indep {e,f}) : e = f :=
  (h.parallel_of_nonloop <| hef.nonloop_of_mem (.inl rfl)).eq_of_indep hef

@[simp] theorem parallel'_iff_parallel [Loopless M] : M.Parallel' e f ↔ M.Parallel e f := by
...
@[simp] theorem isoOfMapParallelBijOn_apply {φ} (hφ : BijOn φ M.E M.E)
    (h_para : ∀ e ∈ M.E, M.Parallel' e (φ e)) (x : M.E) :
    isoOfMapParallelBijOn hφ h_para x = ⟨φ x, (h_para x.1 x.2).mem_ground_right⟩ := rfl

@[simp] theorem isoOfMapParallelBijOn_apply_image {φ} (hφ : BijOn φ M.E M.E)
...
@[simp] theorem isoOfSwapParallel_apply [DecidableEq α] (h_para : M.Parallel' e f) (x : M.E) :
    isoOfSwapParallel h_para x = ⟨Equiv.swap e f x, (isoOfSwapParallel h_para x).2⟩ := rfl

@[simp] theorem isoOfSwapParallel_apply_image [DecidableEq α] (h_para : M.Parallel' e f)
...
theorem indep_iff_range_indep_of_paraMap {φ : I → α} (hφ : φ.Injective)
...
theorem Indep.paraMap_injective (hI : M.Indep I) (φ : I → α)
