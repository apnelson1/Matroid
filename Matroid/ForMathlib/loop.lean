...
def Loop (M : Matroid α) (e : α) : Prop :=
  e ∈ M.cl ∅
pp_extended_field_notation Loop
...
def Nonloop (M : Matroid α) (e : α) : Prop :=
  ¬M.Loop e ∧ e ∈ M.E
pp_extended_field_notation Nonloop

@[reducible] def nonloops (M : Matroid α) : Set α :=
  {e | M.Nonloop e}
pp_extended_field_notation nonloops
...
#align matroid_in.circuit.nonloop_of_mem_of_one_lt_card Matroid.Circuit.nonloop_of_mem_of_one_lt_card
...
#align matroid_in.nonloop_of_not_mem_cl Matroid.nonloop_of_not_mem_cl

/- ./././Mathport/Syntax/Translate/Tactic/Builtin.lean:69:18: unsupported non-interactive tactic ssE -/
theorem nonloop_iff_not_mem_cl_empty
    (he : e ∈ M.E := by aesop_mat) :
    M.Nonloop e ↔ e ∉ M.cl ∅ := by rw [nonloop, loop_iff_mem_cl_empty, and_iff_left he]
#align matroid_in.nonloop_iff_not_mem_cl_empty Matroid.nonloop_iff_not_mem_cl_empty
  ...
  exact
    hfx.trans
      (by
        run_tac
          ssE)
#align matroid_in.nonloop.cl_eq_of_mem_cl Matroid.Nonloop.cl_eq_of_mem_cl
