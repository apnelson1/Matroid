...
@[simps] protected noncomputable def LocalEquiv.ofSetEquiv [Nonempty α] [Nonempty β] {s : Set α}
    {t : Set β} [DecidablePred (· ∈ s)] [DecidablePred (· ∈ t)] (e : s ≃ t) : LocalEquiv α β where
...
@[simp] theorem LocalEquiv.ofSetEquiv_apply_mem [Nonempty α] [Nonempty β] {s : Set α} {t : Set β}
    [DecidablePred (· ∈ s)] [DecidablePred (· ∈ t)] (e : s ≃ t) {x : α} (hx : x ∈ s):
    LocalEquiv.ofSetEquiv e x = e ⟨x,hx⟩ := by
  simp [hx]

end ofSetEquiv

variable [DecidableEq α] [DecidableEq β] {φ φ' : LocalEquiv α β} {a : α} {b : β}

/-- Extend the source/target of a `LocalEquiv` by setting `e a = b`. -/
@[simps] protected def LocalEquiv.insert (e : LocalEquiv α β) (ha : a ∉ e.source)
    (hb : b ∉ e.target) : LocalEquiv α β where
  toFun := update e a b
...
theorem LocalEquiv.insert_apply_mem (e : LocalEquiv α β) (ha : a ∉ e.source) (hb : b ∉ e.target)
    {i : α} (hi : i ∈ e.source) : (e.insert ha hb) i = e i := by
  rw [insert_apply, update_apply, if_neg]
  exact fun h ↦ ha <| h ▸ hi

@[simp] theorem insert_symm (e : LocalEquiv α β) (ha : a ∉ e.source) (hb : b ∉ e.target) :
    (e.insert ha hb).symm = e.symm.insert hb ha := by
  ext <;> simp

theorem LocalEquiv.insert_apply_symm_mem (e : LocalEquiv α β) (ha : a ∉ e.source)
...
instance {α β : Type*} : Preorder (LocalEquiv α β) where
...
theorem LocalEquiv.eq_of_mem_source (h : φ ≤ φ') (ha : a ∈ φ.source) : φ' a = φ a :=
  h.2 ha

theorem LocalEquiv.source_subset (h : φ ≤ φ') : φ.source ⊆ φ'.source :=
  h.1

theorem LocalEquiv.target_subset (h : φ ≤ φ') : φ.target ⊆ φ'.target := by
  rw [← φ.image_source_eq_target, ← φ'.image_source_eq_target]
  rintro _ ⟨x,hx,rfl⟩
  rw [← LocalEquiv.eq_of_mem_source h hx]
  exact mem_image_of_mem _ (source_subset h hx)

theorem LocalEquiv.symm_le_symm_of_le (h : φ ≤ φ') : φ.symm ≤ φ'.symm := by
...
theorem LocalEquiv.symm_le_symm_iff_le : φ.symm ≤ φ'.symm ↔ φ ≤ φ' :=
  ⟨symm_le_symm_of_le, fun h ↦ by simpa using symm_le_symm_of_le h⟩

theorem LocalEquiv.lt_insert (ha : a ∉ φ.source) (hb : b ∉ φ.target) : φ < φ.insert ha hb := by
