...
theorem mul_iSup (c : ℕ∞) (f : α → ℕ∞) : (⨆ i, f i) * c = ⨆ i, (f i * c) := by
...
-- theorem continuous_iff (f : ℕ∞ → ℕ∞) : Continuous f ↔
...
-- instance : TopologicalSemiring ℕ∞ where
...
--   classical
--   simp only [ENat.tsum_eq_iSup_sum, Finset.sum_const, nsmul_eq_mul, mul_one,
--     le_antisymm_iff, iSup_le_iff]
--   refine ⟨fun s₀ ↦ ?_, ?_⟩
--   · rw [← Finset.card_image_of_injOn (Subtype.coe_injective.injOn s₀),
--       ← encard_coe_eq_coe_finsetCard]
--     exact encard_le_of_subset (by simp)
--   obtain (hs | hs) := s.finite_or_infinite
--   · have := hs.fintype
--     convert le_iSup (fun (t : Finset s) ↦ (Finset.card t : ℕ∞)) Finset.univ
--     rw [encard_eq_coe_toFinset_card, toFinset_card, Nat.cast_inj]
--     rfl
--   rw [hs.encard_eq, top_le_iff, iSup_eq_top]
--   rintro b hb
--   lift b to ℕ using hb.ne
--   obtain ⟨t, hts⟩ := hs.exists_subset_card_eq (b+1)
...
protected theorem tsum_mul_left (c : ℕ∞) : c * ∑' a, f a = ∑' a, c * f a := by
  simp_rw [ENat.tsum_eq_iSup_sum]



@[simp] protected theorem tsum_const (s : Set α) (c : ℕ∞) : ∑' (_ : s), c = c * s.encard := by
  simp_rw [← ENat.tsum_one s, ← Summable.tsum_mul_left _ ENat.summable, mul_one]
