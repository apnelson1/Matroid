...
variable {β : Type*} [AddCommMonoid β] [CompleteLattice β] [TopologicalSpace β]
{f g : Set α → ℕ∞}

/-- Given a predicate `P` on `Set α` and a weight function `f` with domain `Set α`
the mininum total weight of a set family with union `y`, in which each set has property `P`.
`P` has a default value of `fun _ ↦ True`. -/
noncomputable def weightedCoverNumber (y : Set α) (f : Set α → β)
    (P : Set α → Prop := fun _ ↦ True) :=
  ⨅ (T : Set (Set α)) (_ : T.IsCover y P), ∑' (x : T), f x.1

lemma IsCover.weightedCoverNumber_le (h : T.IsCover x P) (f : Set α → β) :
...
lemma weightedCoverNumber_sUnion_le (f : Set α → β) (T : Set (Set α)) :
  ...
  IsCover.weightedCoverNumber_le ⟨rfl, by simp⟩ f
...
lemma exists_eq_weightedCoverNumber (h : ∃ T, IsCover T y P) (f : Set α → ℕ∞) :
    ∃ T, IsCover T y P ∧ ∑' (x : T), f x = y.weightedCoverNumber f P :=
  ENat.exists_eq_biInf (s := {T | IsCover T y P}) (f := fun S ↦ ∑' x : S, f x) (by simpa)

lemma exists_eq_weightedCoverNumber' (y : Set α) (f : Set α → ℕ∞) :
    ∃ T, ⋃₀ T = y ∧ ∑' (x : T), f x = y.weightedCoverNumber f := by
  simpa using exists_eq_weightedCoverNumber (y := y) (P := fun _ ↦ True) (f := f) ⟨{y}, by simp⟩

lemma exists_of_weightedCoverNumber_of_ne_top (h : y.weightedCoverNumber f P ≠ ⊤) :
    ∃ T, IsCover T y P ∧ ∑' (x : T), f x = y.weightedCoverNumber f P := by
  simp only [weightedCoverNumber, ne_eq, iInf_eq_top] at h
  exact exists_eq_weightedCoverNumber (by grind) _
...
lemma weightedCoverNumber_iUnion_le_tsum (y : ι → Set α) (f : Set α → ℕ∞) (P : Set α → Prop) :
    (⋃ i, y i).weightedCoverNumber f P ≤ ∑' i, (y i).weightedCoverNumber f P := by
  obtain (h0 | h1) := exists_or_forall_not (fun i ↦ weightedCoverNumber (y i) f P = ⊤)
  · grw [ENat.tsum_eq_top_of_eq_top h0, ← le_top]
  choose S hS using fun i ↦ exists_of_weightedCoverNumber_of_ne_top <| h1 i
  grw [(isCover_iUnion (fun i ↦ (hS i).1)).weightedCoverNumber_le f, ENat.tsum_iUnion_le_tsum,
    tsum_congr (fun i ↦ (hS i).2)]

lemma weightedCoverNumber_sUnion_le_tsum (S : Set (Set α)) (f : Set α → ℕ∞) (P : Set α → Prop) :
    (⋃₀ S).weightedCoverNumber f P ≤ ∑' (x : S), x.1.weightedCoverNumber f P := by
  grw [sUnion_eq_iUnion, weightedCoverNumber_iUnion_le_tsum]

lemma weightedCoverNumber_union_le_add :
    (x ∪ y).weightedCoverNumber f P ≤ x.weightedCoverNumber f P + y.weightedCoverNumber f P := by
  grw [union_eq_iUnion, weightedCoverNumber_iUnion_le_tsum, tsum_bool, cond_false, cond_true,
    add_comm]

/-- A conditional weighted cover number is equal to an unconditional one with
an appropriate function. -/
lemma weightedCoverNumber_eq_weightedCoverNumber_ite (y : Set α) (f : Set α → ℕ∞) (P : Set α → Prop)
    [DecidablePred P] :
    y.weightedCoverNumber f P = y.weightedCoverNumber (fun x ↦ if P x then f x else ⊤) := by
...
/-- If every singleton is admissible, then a cover exists. -/
...
lemma coverNumber_eq_weightedCoverNumber (x : Set α) (P : Set α → Prop) [DecidablePred P] :
  ...
  convert weightedCoverNumber_eq_weightedCoverNumber_ite x 1 P using 1
...
lemma coverNumber_iUnion_le_tsum {ι : Type*} (y : ι → Set α) (P : Set α → Prop) :
  ...
  classical
  simp_rw [coverNumber_eq_weightedCoverNumber, weightedCoverNumber_iUnion_le_tsum]

lemma coverNumber_sUnion_le_tsum (S : Set (Set α)) (P : Set α → Prop) :
    (⋃₀ S).coverNumber P ≤ ∑' (x : S), x.1.coverNumber P := by
  grw [sUnion_eq_iUnion, coverNumber_iUnion_le_tsum]
...
lemma coverNumber_union_le {P Q : Set α → Prop} {x : Set α} {y : Set α}
...
lemma coverNumber_union_le_add : (x ∪ y).coverNumber P ≤ x.coverNumber P + y.coverNumber P := by
  grw [union_eq_iUnion, coverNumber_iUnion_le_tsum, tsum_bool, cond_false, cond_true, add_comm]
