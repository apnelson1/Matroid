...
def ENat.multichoose (n : ℕ∞) (k : ℕ) : ℕ∞ :=
  match n, k with
  | some n, k => Nat.multichoose n k
  | none, 0 => 1
  | none, _ + 1 => ⊤
...
-- @[simp]
-- lemma ENat.multichoose_zero (n : ℕ∞) : ENat.multichoose n 0 = 1 := by



instance : IsAddTorsionFree ℕ∞ where
  nsmul_right_injective n hn i j hij := by
    simp only [nsmul_eq_mul] at hij
    exact (ENat.mul_right_strictMono (a := n) (by simpa) (by simp)).injective hij

instance ENat.instBinomialRing : BinomialRing ℕ∞ where
  multichoose := ENat.multichoose
  toIsAddTorsionFree := by infer_instance
  factorial_nsmul_multichoose r n := by
    have h0 : (⊤ : ℕ∞).multichoose 0 = 1 := rfl
    have h1 (n : ℕ) : (⊤ : ℕ∞).multichoose (n + 1) = ⊤ := rfl
    cases r using ENat.recTopCoe with
    | top =>
      simp only [nsmul_eq_mul, ascPochhammer_smeval_cast]
      induction n with
      | zero => simp [h0]
      | succ n ih =>
        rw [Nat.factorial_succ, Nat.cast_mul, Nat.cast_add, Nat.cast_one,
          ascPochhammer_succ_left, mul_comm X, smeval_mul_X, smeval_comp, h1]
        simp only [smeval_add, smeval_X, pow_one, smeval_one, pow_zero, one_smul, top_add]
        rw [← ih, mul_top (by simp [Nat.factorial_ne_zero]), mul_top]
...
def choose [AddCommMonoidWithOne R] [Pow R ℕ] [BinomialRing R] (r : R) (n : ℕ) : R :=
  multichoose (r - n + 1) n
