import Matroid.SimpGraph.SimpleBasic
...
variable {α β : Type*} {x y z u v w : α} {a b c d : β} {e f : Sym2 β} {G H : Graph α β}
  {F : Set (Sym2 β)} {S T X Y : Set α}
...
@[simp]
lemma IsLink.other_eq (h : G.IsLink e x y) : Inc.other ⟨y, h⟩ = y :=
  h.inc_left.choose_spec.right_unique h
...
lemma isLink_eq_isLink_iff_exists_isLink_of_mem_edgeSet (heG : e ∈ E(G)) :
  ...
  · simp only [← h, and_self]
    exact G.mem_edgeSet_iff_exists_isLink.mp heG
  · ext u v
    rw [hG.isLink_iff_sym2_eq, hH.isLink_iff_sym2_eq]

/-- The set of ends of an edge `e`. -/
def endSet (G : Graph α β) (e : Sym2 β) : Set α := {x | G.Inc e x}
...
@[simp, grind! .]
lemma endSet_encard_le_two (G : Graph α β) (e : Sym2 β) : V(G, e).encard ≤ 2 := by
  by_cases heE : e ∈ E(G)
...
@[grind]
def incVertexSet (G : Graph α β) (F : Set (Sym2 β)) : Set α := {x | ∃ e ∈ F, G.Inc e x}
...
@[simp, grind! .]
lemma incVertexSet_subset (G : Graph α β) (F : Set (Sym2 β)) : V(G, F) ⊆ V(G) := by
  rintro x ⟨e, hS, he⟩
  exact he.vertex_mem

lemma incVertexSet_eq_sUnion (G : Graph α β) (F : Set (Sym2 β)) : V(G, F) = ⋃ e ∈ F, V(G, e) := by
  ext x; simp

lemma incVertexSet_encard_le (G : Graph α β) (F : Set (Sym2 β)) :
    V(G, F).encard ≤ 2 * F.encard := by
...
lemma incVertexSet_inter_edgeSet (G : Graph α β) (F : Set (Sym2 β)) : V(G, F ∩ E(G)) = V(G, F) := by
  ext x
...
@[grind]
def parallel (G : Graph α β) (e f : Sym2 β) : Prop :=
...
lemma parallel.trans {g : Sym2 β} (h : G.parallel e f) (h' : G.parallel f g) : G.parallel e g := by
  obtain ⟨he, hf, h⟩ := h
  obtain ⟨hf, hg, h'⟩ := h'
...
def parallelClasses (G : Graph α β) : Partition (Set (Sym2 β)) :=
...
@[grind]
def Neighbor (G : Graph α β) (x : α) : Set α := {y | Adj G x y}
...
@[simp]
lemma notMem_neighbor_of_not_adj (hadj : ¬ Adj G x y) : y ∉ N(G, x) := by grind

lemma neighbor_subset_of_ne_not_adj (hne : x ≠ y) (hadj : ¬ Adj G x y) :
    N(G, x) \ {x} ⊆ V(G) \ {x, y} := by
  rintro z ⟨hz, hne⟩
  rw [mem_singleton_iff] at hne
  simp only [mem_diff, mem_insert_iff, hne, mem_singleton_iff, false_or]
  use Adj.right_mem hz
  rintro rfl
  exact hadj hz
...
@[grind]
def SetNeighbor (G : Graph α β) (S : Set α) : Set α := {y | y ∉ S ∧ ∃ x ∈ S, Adj G x y}
...
@[simp]
lemma notMem_setNeighbor_of_not_adj (hadj : ¬ Adj G x y) : y ∉ N(G, {x}) := by
  simp [SetNeighbor, hadj]

/-- The set of edges incident to vertex `v` in `G`. -/
@[grind]
def IncEdges (G : Graph α β) (v : α) : Set (Sym2 β) := {e | G.Inc e v}
...
@[simp, grind =]
lemma mem_incEdges_iff (G : Graph α β) (v : α) (e : Sym2 β) : e ∈ E(G, v) ↔ G.Inc e v := Iff.rfl

/-- The set of edges incident to at least one vertex in `S`. -/
@[grind]
def SetIncEdges (G : Graph α β) (S : Set α) : Set (Sym2 β) := {e | ∃ x ∈ S, G.Inc e x}
...
@[grind]
def LinkEdges (G : Graph α β) (u v : α) : Set (Sym2 β) := {e | G.IsLink e u v}

notation "E(" G ", " u ", " v ")" => LinkEdges G u v

@[simp]
lemma linkEdges_empty (G : Graph α β) (u v : α) : E(G, u, v) = ∅ ↔ ¬ Adj G u v := by
  simp [LinkEdges, adj_iff_exists_isLink, Set.ext_iff]

@[simp]
lemma linkEdges_self (G : Graph α β) (u : α) : E(G, u, u) = {e | G.IsLoopAt e u} := rfl

@[simp, grind =]
lemma mem_linkEdges_iff (G : Graph α β) (u v : α) (e : Sym2 β) : e ∈ E(G, u, v) ↔ G.IsLink e u v :=
  Iff.rfl
...
@[grind]
def SetLinkEdges (G : Graph α β) (S T : Set α) : Set (Sym2 β) :=
  {e | ∃ x ∈ S, ∃ y ∈ T, G.IsLink e x y}
...
@[grind =]
lemma mem_setLinkEdges_iff (G : Graph α β) (S T : Set α) (e : Sym2 β) :
...
@[simp]
lemma Isolated.not_adj (h : G.Isolated x) : ¬ Adj G x y := by
  rw [adj_iff_exists_isLink]
  exact fun ⟨_, he⟩ ↦ h.not_inc he.inc_left
...
@[mk_iff, grind]
structure IsPendant (G : Graph α β) (e : Sym2 β) (x : α) : Prop where
  isNonloopAt : G.IsNonloopAt e x
  edge_unique : ∀ ⦃f⦄, G.Inc f x → f = e

lemma IsPendant.not_isLoopAt (h : G.IsPendant e x) (f : Sym2 β) : ¬ G.IsLoopAt f x := by
...
lemma IsLeaf.exists_unique_adj (h : G.IsLeaf x) : ∃! y, Adj G x y := by
  obtain ⟨e, ⟨y, he⟩, hunique⟩ := h.exists_unique_inc
  refine ⟨y, he.adj, fun z hz ↦ ?_⟩
  obtain ⟨f, hz⟩ := adj_iff_exists_isLink.mp hz
  rw [hunique f hz.inc_left] at hz
  exact hz.right_unique he

lemma IsLeaf.eq_of_adj_adj (h : G.IsLeaf x) (hu : Adj G x u) (hv : Adj G x v) : u = v := by
...
lemma IsLeaf.not_adj_self (h : G.IsLeaf x) : ¬ Adj G x x := by
  rw [adj_iff_exists_isLink]
  rintro ⟨e, he : G.IsLoopAt e x⟩
  obtain ⟨f, h⟩ := h
  exact h.not_isLoopAt e he

lemma IsLeaf.ne_of_adj (h : G.IsLeaf x) (hxy : Adj G x y) : x ≠ y :=
...
@[grind] def IsLeafEdge (G : Graph α β) (e : Sym2 β) := ∃ x, G.IsPendant e x
