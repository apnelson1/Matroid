...
theorem twoPaths [DecidableEq V] {G : SimpleGraph V} {u v : V} {P₁ P₂ : G.Walk u v}
    (hP₁ : P₁.IsPath) (hP₂ : P₂.IsPath) (hne : P₁ ≠ P₂) :
    ¬ (fromEdgeSet (P₁.edges.toFinset.toSet ∪ P₂.edges.toFinset.toSet)).IsAcyclic := sorry

lemma Connected.exists_adj [Nontrivial V] (hG : G.Connected) (v : V) : ∃ w, G.Adj v w := by
  obtain ⟨x, hx⟩ := exists_ne v
  obtain ⟨P, hP⟩ := hG.exists_isPath x v
  induction P with
  | nil => simp at hx
  | @cons a b c h p ih =>
  simp only [Walk.cons_isPath_iff] at hP
  obtain rfl | ne := eq_or_ne b c
  · exact ⟨a, h.symm⟩
  exact ih ne hP.1
...
--   have :=
  -- have : H.coe ≃g H.spanningCoe := by
  --   apply?
  -- rw [Subgraph.connected_iff_forall_exists_walk_subgraph,
  --   SimpleGraph.connected_induce_iff]

  -- rw [connected_iff_exists_forall_reachable, Subgraph.connected_iff_forall_exists_walk_subgraph]


theorem Minimal_Connected_Is_Spanning_Tree (G : SimpleGraph V) {H : G.Subgraph}
    (hmin : Minimal (fun X ↦ X.spanningCoe.Connected) H) : H.IsSpanningTree := by
  refine ⟨hmin.prop, (isTree_of_minimal_connected (G := H.spanningCoe) ?_).IsAcyclic⟩

  sorry
  -- unfold Minimal_Connected at h
  -- by_contra! co
  -- obtain ⟨h_conn, h_min⟩ := h
  -- rw[Is_Spanning_Tree] at co
  -- push_neg at co
  -- apply co at h_conn
  -- rw [isAcyclic_iff_forall_adj_isBridge] at h_conn
  -- push_neg at h_conn
  -- obtain ⟨v, w, he_edge, he_not_bridge⟩ := h_conn
  -- have newh : ¬ (H.spanningCoe.Adj v w ∧ ¬(H.spanningCoe \ fromEdgeSet {s(v, w)}).Reachable v w) := by
  --   by_contra! co_new
  --   apply isBridge_iff.mpr at co_new
  --   exact he_not_bridge co_new
  -- push_neg at newh
  -- apply newh at he_edge
  -- have remcon : (H.spanningCoe.deleteEdges {s(v,w)}).Connected := by
  --   apply Connected.connected_delete_edge_of_not_isBridge
  --   sorry
  --   sorry
  -- sorry
