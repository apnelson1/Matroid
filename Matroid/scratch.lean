...
def Path.multitrans (x : Fin (n+1) → X) (P : ∀ i : Fin n, Path (x i.castSucc) (x i.succ)) :
  Path (x 0) (x (Fin.last n)) := sorry

lemma Path.multitrans_reverse (x : Fin (n+1) → X)
    (P : ∀ i : Fin n, Path (x i.castSucc) (x i.succ)) :
  Path.multitrans (Fin.rev ∘ x) (fun )
