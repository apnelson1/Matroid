...
(which can be different for the LHS and RHS if needed),
...
If `w` is a function assigning a `ℕ∞`-weight to each set `X`
...
this gives Tutte `k`-connectivity with `w M = M.tutteWeight`,
it gives vertical `k`-connectivity with `w M = M✶.nullity`,
and cyclical `k`-connectivity with `w M = M.nullity`.

If `f = (0, 0, ..., 1, ⊤, ⊤, ⊤, ...)` with the `1` in position `(k-2)`,
and `w M = M.tutteWeight`, then this is internal `k`-connectivity. -/
