...
--     {C₀ : Set α} (hC₀ : (M ＼ (D \ B)).IsCircuit C₀) (hf : f ∈ C₀) (hfP : f ∈ P !i) (hfN : f ∉ N.E)
--     (hfC : f ∉ C) : C₀ ⊆ B ∪ C := by
--   rw [delete_isCircuit_iff] at hC₀
--   have h1 : (C₀ ∩ (C \ B)).Nonempty := by

--     by_contra! hcon
--     rw [← disjoint_iff_inter_eq_empty] at hcon
--     have hDep : (M ／ P i).Dep (C₀ ∩ P !i) := by
--       rw [← Separation.diff_eq_inter_bool]
--       refine hC₀.1.contract_dep_of_not_subset fun hCi ↦ ?_
--       sorry -- use hfP
--     refine (hB.indep.subset ?_).not_dep hDep
--     have hss : P (!i) ⊆ C ∪ D := by
--       rw [hCDP]
