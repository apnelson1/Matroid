...
/-- A (supported) set is `Large` in a tangle if its complement is small. -/
@[mk_iff]
...
/-- The union of two small sets, if its connectivity is low enough, is small. -/
...
lemma Large.inter_large (hX : T.Large X) (hY : T.Large Y) (hXY : μ (X ∩ Y) ≤ k) :
  ...
  rwa [← sdiff_inter, apply_compl, imp_iff_right hXY] at h
...
/-- Given a tangle `T` of order `k` for a connectivity system `ν`, the tangle of order `k`
in a system `μ ≤ ν` whose small sets are those that intersect `ν.E` in a small set. -/
@[simps]
def induce {μ ν : ConnSystem α R} (T : Tangle ν k) (h_le : ν ≤ μ) : Tangle μ k where
  ...
  small_or_small_compl X hXE hXk := by
    rw [and_iff_right hXE, and_iff_right sdiff_subset, μ.apply_compl, and_iff_right hXk,
