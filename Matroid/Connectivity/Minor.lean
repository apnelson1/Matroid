...
lemma multiConn_dual_eq_eRank_project (hdj : Pairwise (Disjoint on X)) (hu : ⋃ i, X i = M.E)
  ...
  rw [multiConn_eq_nullity_iUnion'' hdj (fun i ↦ (hI' i).isBasis'), nullity_eq_eRank_restrict_dual,
    ← delete_compl (by grw [iUnion_mono (fun _ ↦ sdiff_subset), hu, dual_ground]),
...
lemma Coindep.delete_eConn_eq_union_iff' (hD : M.Coindep D) (hDX : Disjoint X D)
  ...
  wlog hXE : X ⊆ M.E generalizing X with aux
  · rw [← closure_inter_ground, ← aux (hDX.mono_left inter_subset_left) _ inter_subset_right,
    ...
    · convert Iff.rfl using 3
      ...
      · ext x
        ...
        exact ⟨fun ⟨h, hxE⟩ ↦ ⟨h.elim (fun hx ↦ .inl hx.1) .inr, hxE⟩,
          fun ⟨h, hxE⟩ ↦ ⟨h.elim (fun hx ↦ .inl ⟨hx, hxE⟩) .inr, hxE⟩⟩
  ...
  grw [M.eConn_eq_eConn_delete_subset_add (X := X ∪ D) subset_union_right, union_sdiff_right,
    ...
    skew_comm, hD.skew_dual_iff (by grind), union_comm, ← Set.sdiff_sdiff,
...
lemma eLocalConn_le_dual_eRelRk (M : Matroid α) (hXY : Disjoint X Y) :
  ...
  nth_rw 1 [eRelRk_eq_eRk_diff_contract,
    ...
    ← eConn_eq_eLocalConn, ← eConn_dual, dual_delete, sdiff_sdiff_comm, hY, delete_ground,
     sdiff_sdiff_cancel_left (by grind)]
  grw [eConn_le_eRk ..]
...
lemma IsMinor.isRestriction_of_eConn_eq_zero {N} (hN : N ≤m M) (hN0 : M.eConn N.E = 0) :
  ...
  · simp
  · suffices M.Skew C ((M.E \ C) \ D) by simpa [delete_ground, sdiff_sdiff_comm, skew_delete_iff,
