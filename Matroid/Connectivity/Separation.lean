...
@[simps] protected def copy {M' : Matroid α} (P : M.Partition) (h_eq : M = M') : M'.Partition where
  left := P.left
  right := P.right
  disjoint := P.disjoint
...
def Partition.SepOf (P : M.Partition) (X Y : Set α) : Prop := X ⊆ P.left ∧ Y ⊆ P.right
...
lemma Partition.sepOf_self_compl_iff (hX : X ⊆ M.E) :
  ...
  rw [SepOf, diff_subset_comm, P.compl_right, ← subset_antisymm_iff]
...
/-- The 'kappa' connectivity between `X` and `Y`.
Defined to be unchanged by adding/removing junk outside the ground set  -/
noncomputable def eConnBetween (M : Matroid α) (X Y : Set α) : ℕ∞ :=
  ⨅ P : {P : M.Partition // P.SepOf (X ∩ M.E) (Y ∩ M.E)}, P.1.eConn

lemma Partition.SepOf.eConnBetween_le_of_inter (hP : P.SepOf (X ∩ M.E) (Y ∩ M.E)) :
    M.eConnBetween X Y ≤ P.eConn :=
  iInf_le_of_le ⟨P, hP⟩ rfl.le

lemma Partition.SepOf.eConnBetween_le (hP : P.SepOf X Y) :
    M.eConnBetween X Y ≤ P.eConn :=
  (hP.mono inter_subset_left inter_subset_left).eConnBetween_le_of_inter

lemma le_eConnBetween_iff_forall_sepOf_inter {k : ℕ∞} : k ≤ M.eConnBetween X Y ↔
    ∀ (P : M.Partition), P.SepOf (X ∩ M.E) (Y ∩ M.E) → k ≤ P.eConn := by
  simp [eConnBetween, le_iInf_iff, Subtype.forall]

lemma le_eConnBetween_iff_forall_sepOf {k : ℕ∞} (hX : X ⊆ M.E) (hY : Y ⊆ M.E) :
    k ≤ M.eConnBetween X Y ↔ ∀ (P : M.Partition), P.SepOf X Y → k ≤ P.eConn := by
  simp_rw [le_eConnBetween_iff_forall_sepOf_inter, inter_eq_self_of_subset_left hX,
    inter_eq_self_of_subset_left hY]

@[simp]
lemma eConnBetween_inter_ground_left (M : Matroid α) (X Y : Set α) :
    M.eConnBetween (X ∩ M.E) Y = M.eConnBetween X Y := by
  simp [eConnBetween, iInf_subtype, inter_assoc]
...
lemma exists_partition_eConn_eq_eConnBetween_inter (M : Matroid α)
    (hXY : Disjoint (X ∩ M.E) (Y ∩ M.E)) :
    ∃ P : M.Partition, P.SepOf (X ∩ M.E) (Y ∩ M.E) ∧ P.eConn = M.eConnBetween X Y := by
  have hne : (M.partition (X ∩ M.E)).SepOf (X ∩ M.E) (Y ∩ M.E) := by
    suffices Y ∩ M.E ∩ X = ∅ by simpa [Partition.SepOf, subset_diff, disjoint_iff_inter_eq_empty]
    rwa [inter_right_comm, inter_inter_distrib_right, ← disjoint_iff_inter_eq_empty, disjoint_comm]
  have : Nonempty {P : M.Partition // P.SepOf (X ∩ M.E) (Y ∩ M.E)} := ⟨_, hne⟩
  obtain ⟨⟨P,h⟩, hP⟩ := ENat.exists_eq_iInf (α := {P : M.Partition // P.SepOf (X ∩ M.E) (Y ∩ M.E)})
    (f := fun i ↦ i.1.eConn)
  exact ⟨P, h, hP⟩

lemma exists_partition_eConn_eq_eConnBetween (M : Matroid α) (hXY : Disjoint X Y)
    (hXE : X ⊆ M.E) (hYE : Y ⊆ M.E) :
    ∃ P : M.Partition, P.SepOf X Y ∧ P.eConn = M.eConnBetween X Y :=
  let ⟨P, hP⟩ := M.exists_partition_eConn_eq_eConnBetween_inter <|
    hXY.mono inter_subset_left inter_subset_left
  ⟨P, by rwa [inter_eq_self_of_subset_left hXE, inter_eq_self_of_subset_left hYE] at hP⟩

lemma eConnBetween_of_not_disjoint (M : Matroid α) (hXY : ¬ Disjoint (X ∩ M.E) (Y ∩ M.E)) :
    M.eConnBetween X Y = ⊤ := by
  simp [eConnBetween, iInf_subtype, show ∀ P : M.Partition, ¬ P.SepOf (X ∩ M.E) (Y ∩ M.E) from
...
lemma eConnBetween_mono_left (M : Matroid α) (hX : X' ⊆ X) (Y : Set α) :
  ...
  le_iInf fun ⟨P, hP⟩ ↦ iInf_le_of_le ⟨P, hP.mono_left (inter_subset_inter_left _ hX)⟩ rfl.le

lemma eConnBetween_mono_right (M : Matroid α) (X : Set α) (hY : Y' ⊆ Y) :
    M.eConnBetween X Y' ≤ M.eConnBetween X Y :=
  le_iInf fun ⟨P, hP⟩ ↦ iInf_le_of_le ⟨P, hP.mono_right (inter_subset_inter_left _ hY)⟩ rfl.le

lemma eConnBetween_mono (M : Matroid α) (hX : X' ⊆ X) (hY : Y' ⊆ Y) :
    M.eConnBetween X' Y' ≤ M.eConnBetween X Y :=
  (M.eConnBetween_mono_left hX _).trans <| M.eConnBetween_mono_right _ hY

lemma eConnBetween_self_compl (M : Matroid α) (hX : X ⊆ M.E) :
    M.eConnBetween X (M.E \ X) = M.eConn X := by
  simp [eConnBetween, inter_eq_self_of_subset_left hX, iInf_subtype,
    inter_eq_self_of_subset_left diff_subset, Partition.sepOf_self_compl_iff hX]

lemma eConnBetween_compl_self (M : Matroid α) (hX : X ⊆ M.E) :
    M.eConnBetween (M.E \ X) X = M.eConn X := by
  rw [eConnBetween_symm, M.eConnBetween_self_compl hX]

@[simp]
lemma eConnBetween_removeLoops_eq (M : Matroid α) :
    M.removeLoops.eConnBetween = M.eConnBetween := by
  ext X Y
  obtain hdj | hndj := em' <| Disjoint (X ∩ M.removeLoops.E) (Y ∩ M.removeLoops.E)
  · rw [eConnBetween_of_not_disjoint _ hdj, eConnBetween_of_not_disjoint]
    refine fun h ↦ hdj <| h.mono ?_ ?_ <;>
    exact inter_subset_inter_right _ M.removeLoops_restriction.subset



  have := M.removeLoops.exists_partition_eConn_eq_eConnBetween_inter X Y

  -- ext X Y
  -- simp only [le_antisymm_iff, le_eConnBetween_iff_forall_sepOf_inter, removeLoops_ground_eq]
  -- refine ⟨fun P hP ↦ ?_, fun h ↦ ?_⟩
  -- · have := hP.eConnBetween_le
