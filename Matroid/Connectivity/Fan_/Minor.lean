...
lemma IsFan.contract (hF : M.IsFan F b c) (X : Set α) (hX : _root_.Disjoint {e | e ∈ F} X)
    (h0 : b = false → ∀ (h : 0 < F.length), F[0] ∉ M.closure X)
    (hlast : c = false → ∀ (h : 0 < F.length), F[F.length - 1] ∉ M.closure X)
    (h3 : F.length = 3 → b = false → c = false → M.Skew {e | e ∈ F} (X ∩ M.E) := by lia) :
    (M ／ X).IsFan F b c := by
  have hFX : ∀ {i} {hi : i < F.length}, F[i] ∉ X := by grind
  refine isFan_of_forall_isCircuit hF.nodup (by simp [hF.length_bodd_eq])
    ?_ fun i hi ↦ ?_
  · rintro hF2 i hi
    · obtain rfl | rfl := b
      ...
      have h1cl : F[1] ∉ M.closure X := by
        simpa [hF.bool_right_eq, show F.length = 2 by lia] using hlast
  ...
  · grw [insert_comm, insert_union, subset_insert_iff_of_notMem hi1, pair_comm,
    ...
    exact h0 (by simpa) (by lia) <| mem_of_mem_of_subset (hC.mem_closure_sdiff_singleton_of_mem hiC)
...
lemma IsFan.minor {N : Matroid α} (hF : M.IsFan F b c) (h4 : 4 ≤ F.length) (hNM : N ≤m M)
  ...
  · have hwin := (hCF.dual.contract (X := D) (by grind) ?_ ?_).dual
    ...
    · simp only [Bool.not_eq_eq_eq_not, Bool.not_false, dual_contract, delete_closure_eq, mem_sdiff,
      ...
      rintro rfl hcl
      simp [hCD.sdiff_eq_right] at ⊢ h_first
      exact h_first.2
    simp only [Bool.not_eq_eq_eq_not, Bool.not_false, dual_contract, delete_closure_eq, mem_sdiff]
    rintro rfl hlt hcl
    simp [hcl.1, hcl.2] at h_last
  · rintro rfl hcl
...
lemma isFan_delete_iff_of_subset_loops {X : Set α} (hX : X ⊆ M.loops)
  ...
  convert Iff.rfl using 6 with a i i
...
lemma IsFan.restrict (hF : M.IsFan F b c) (X : Set α) (hX : {e | e ∈ F} ⊆ X)
    (h0 : b = true → ∀ (h0 : 0 < F.length), F[0] ∈ M.closure (X \ {F[0]}))
    (hlast : c = true → ∀ (h0 : 0 < F.length), F[F.length - 1] ∈ M.closure (X \ {F[F.length - 1]}))
    (h3 : F.length = 3 → b = true → c = true → M✶.Skew {e | e ∈ F} (M.E \ X) := by lia) :
    (M ↾ X).IsFan F b c := by
  obtain rfl | hne := eq_or_ne F []
  · simpa using hF
  simp only [length_pos_iff_ne_nil.2 hne, forall_true_left] at h0 hlast
  ...
  · simp only [Bool.not_eq_eq_eq_not, Bool.not_false, length_pos_iff_ne_nil.2 hne,
      forall_true_left]
    rwa [mem_dual_closure_iff_notMem_closure_compl, sdiff_sdiff_cancel_left hXE, not_not]
...
lemma IsFan.contract_head (hF : M.IsFan F b c) (h0 : F ≠ [])
    (h_init : ∀ (h2 : 2 ≤ F.length), b = true → ¬ M.Parallel F[0] F[1])
    (h_false : ∀ (h5 : 5 ≤ F.length), b = false → c = false → ¬ M.Parallel F[0] F[F.length - 1])
    (h4 : ∀ (h4 : F.length = 4), b = true → ¬ F[0] ∈ M.closure {F[1], F[2]} := by lia) :
    (M ／ {F[0]}).IsFan F.tail (!b) c := by
  obtain h2 | h3 := lt_or_ge F.length 3
  · match F with
    | [] => simp at h0
    | [e] => simp [hF.bool_right_eq]
    | [e, f] =>
      obtain rfl | rfl := b
      · simpa [hF.bool_right_eq, show f ≠ e by simpa [ne_comm] using hF.nodup]
          using hF.isNonloop_right
      simpa [parallel_comm (e := e),
        IsNonloop.parallel_iff_mem_closure (by simpa using hF.isNonloop_right),
        hF.bool_right_eq, show f ∈ M.E from hF.subset_ground (by simp)] using h_init
  have aux := @IsFan.contract _ M F.tail _ _ (hF.tail h0) {F[0]}
    (by simp [getElem_zero_eq_head, hF.nodup.head_notMem_tail])
  simp only [Bool.not_eq_eq_eq_not, Bool.not_false, length_tail, tsub_pos_iff_lt,
    show 1 < F.length by lia, getElem_tail, zero_add,
    ← IsNonloop.parallel_iff_mem_closure (hF.isNonloop_getElem 1 (by lia)),
    parallel_comm (f := F[0]), forall_const, show F.length - 1 - 1 + 1 = F.length - 1 by lia,
    ← IsNonloop.parallel_iff_mem_closure (hF.isNonloop_getElem (F.length - 1) (by lia)),
    Nat.pred_eq_succ_iff, Nat.reduceAdd, singleton_inter_of_mem hF.getElem_mem_ground,
    imp_iff_right (h_init (by lia))] at aux
  refine aux ?_ ?_
  · rintro rfl hpara
    obtain rfl | rfl := b
    · obtain h3 | h4 | h5 : (F.length = 3 ∨ F.length = 4 ∨ 5 ≤ F.length) := by lia
      · have hcon := (hF.isTriangle_getElem_of_eq 0 rfl).notMem_of_mem_of_parallel hpara
        simp [h3, hF.nodup.getElem_inj_iff] at hcon
      · simpa [h4] using hF.length_bodd_eq
      exact h_false h5 rfl rfl hpara
  ...
  rintro hF4 rfl rfl
  rw! [(hF.isNonloop_getElem 0 (by lia)).skew_right_iff (hF.tail h0).subset_ground,
...
lemma IsFan.delete_head (hF : M.IsFan F b c) (h0 : F ≠ [])
    (h_init : ∀ (h2 : 2 ≤ F.length), b = false → ¬ M✶.Parallel F[0] F[1])
    (h_pair : ∀ (h5 : 5 ≤ F.length), b = true → c = true → ¬ M✶.Parallel F[0] F[F.length - 1])
    (h4 : ∀ (h4 : F.length = 4), b = false → ¬ F[0] ∈ M✶.closure {F[1], F[2]} := by lia) :
    (M ＼ {F[0]}).IsFan F.tail (!b) c := by
  simpa using (hF.dual.contract_head (by lia) (by simpa) (by simpa)).dual

lemma IsFan.remove_head (hF : M.IsFan F b c) (h0 : F ≠ []) {d : Bool}
    (h_init : ∀ (h2 : 2 ≤ F.length), b = d → ¬ (M.bDual !d).Parallel F[0] F[1])
    (h_pair : ∀ (h2 : 5 ≤ F.length), b = !d → c = !d → ¬ (M.bDual !d).Parallel F[0] F[F.length - 1])
    (h4 : ∀ (h4 : F.length = 4), b = d → ¬ F[0] ∈ (M.bDual !d).closure {F[1], F[2]} := by lia) :
    (M.remove d {F[0]}).IsFan F.tail (!b) c := by
  obtain rfl | rfl := d
  · exact hF.delete_head h0 (by simpa) (by simpa) (by simpa using h4)
  exact hF.contract_head h0 (by simpa) (by simpa) (by simpa using h4)
