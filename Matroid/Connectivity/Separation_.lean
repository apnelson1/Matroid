...
-- def TutteDegen (M : Matroid α) (X : Set α) : Prop := X.encard ≤

def TutteConnected (M : Matroid α) (k : ℕ∞) : Prop := ∀ (P : M.Partition),
    P.eConn + 2 ≤ k → P.left.encard ≤ P.eConn ∨ P.right.encard ≤ P.eConn

-- def InternallyDegen (M : Matroid α) (k : ℕ∞) (X : Set α) := X.encard + 1 ≤ k ∧

def InternallyConnected (M : Matroid α) (k : ℕ∞) : Prop := ∀ (P : M.Partition),
    (P.eConn + 3 ≤ k → P.left.encard ≤ P.eConn ∨ P.right.encard ≤ P.eConn) ∧
    (P.eConn + 2 ≤ k → P.left.encard ≤ P.eConn + 1 ∨ P.right.encard ≤ P.eConn + 1)

def VerticallyConnected (M : Matroid α) (k : ℕ∞) : Prop := ∀ (P : M.Partition),
    P.eConn + 2 ≤ k → M.eRk P.left ≤ P.eConn ∧ M.eRk P.right ≤ P.eConn

lemma TutteConnected.mono (hM : M.TutteConnected k) (hjk : j ≤ k) : M.TutteConnected j :=
  fun P hP ↦ hM P <| hP.trans hjk

lemma InternallyConnected.mono (hM : M.InternallyConnected k) (hjk : j ≤ k) :
    M.InternallyConnected j :=
  fun P ↦ ⟨fun h ↦ (hM P).1 (h.trans hjk), fun h ↦ (hM P).2 (h.trans hjk)⟩

lemma TutteConnected.internallyConnected (hM : M.TutteConnected k) : M.InternallyConnected k := by
  refine fun P ↦ ⟨fun h ↦ hM P (le_trans ?_ h), fun h ↦ ?_⟩
  · generalize h' : P.eConn = j
    enat_to_nat <;> linarith
  obtain h1 | h1 := hM P h
  · exact .inl <| h1.trans le_self_add
  exact .inr <| h1.trans le_self_add

def InternallyConnected.tutteConnected (hM : M.InternallyConnected (k+1)) : M.TutteConnected k := by
  refine fun P hP ↦ (hM P).1 ?_
  grw [← hP, add_assoc]
  norm_num



    -- P.eConn + 2 < k → P.left.encard ≤ P.eConn ∨ P.
    -- (P.left.encard P.left.encard ≤ P.eConn ∨ P.right.encard ≤ P.eConn

-- def InternallyConnected (M : Matroid α) (k : ℕ∞) := ∀ (P : M.Partition),
--     P.eConn < k → P.l
