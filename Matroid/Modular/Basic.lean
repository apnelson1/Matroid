...
lemma ModularBase.isBasis_inter (h : M.ModularBase B Xs) (i : ι) : M.IsBasis ((Xs i) ∩ B) (Xs i) :=
  h.2 i
...
lemma ModularBase.isBasis_iInter [Nonempty ι] (h : M.ModularBase B Xs) :
    M.IsBasis ((⋂ i, Xs i) ∩ B) (⋂ i, Xs i) :=
  h.1.indep.inter_IsBasis_iInter (fun _ ↦ h.2 _)

lemma ModularBase.isBasis_iUnion (h : M.ModularBase B Xs) :
...
lemma IsBase.modularBase_of_forall_subset_closure (hB : M.IsBase B)
...
lemma ModularBase.isBasis_biInter {Xs : η → Set α} (h : M.ModularBase B Xs) (hA : A.Nonempty) :
...
lemma ModularBase.isBasis_biUnion {Xs : η → Set α} (h : M.ModularBase B Xs) (A : Set η) :
...
lemma ModularBase.switch (hB : M.ModularBase B Xs) (hIX : M.IsBasis I (⋂ i, Xs i)) :
...
lemma Indep.modularFamily (hI : M.Indep I) (hXs : ∀ i, M.IsBasis ((Xs i) ∩ I) (Xs i)) :
...
lemma ModularFamily.contract (h : M.ModularFamily Xs) {C : Set α} (hC : ∀ i, C ⊆ M.closure (Xs i)) :
  ...
  refine hi.modularFamily fun i ↦ (hi.inter_left _).isBasis_of_subset_of_subset_closure
    inter_subset_left ?_
...
lemma ModularFamily.finite_of_forall_isFlat [M.RankFinite] (h : M.ModularFamily Xs)
...
lemma modularPair_iff {M : Matroid α} {X Y : Set α} :
    M.ModularPair X Y ↔ ∃ I, M.Indep I ∧ M.IsBasis (X ∩ I) X ∧ M.IsBasis (Y ∩ I) Y := by
...
lemma ModularFamily.modularPair (h : M.ModularFamily Xs) (i j : ι) :
  ...
  obtain ⟨B, hB⟩ := h
  exact modularPair_iff.2 ⟨B, hB.indep, hB.isBasis_inter i, hB.isBasis_inter j⟩
...
lemma modularPair_iff_exists_isBasis_isBasis :
    M.ModularPair X Y ↔ ∃ I J, M.IsBasis I X ∧ M.IsBasis J Y ∧ M.Indep (I ∪ J) := by
  rw [modularPair_iff]
...
lemma ModularPair.of_isBasis_left (h : M.ModularPair I Y) (hIX : M.IsBasis I X) :
    M.ModularPair X Y :=
  h.of_subset_closure_left hIX.subset hIX.subset_closure

lemma ModularPair.of_isBasis_right (h : M.ModularPair X J) (hJY : M.IsBasis J Y) :
    M.ModularPair X Y :=
  h.of_subset_closure_right hJY.subset hJY.subset_closure

lemma ModularPair.of_isBasis_of_isBasis (h : M.ModularPair I J) (hIX : M.IsBasis I X)
    (hJY : M.IsBasis J Y) : M.ModularPair X Y :=
...
lemma modularPair_singleton (he : e ∈ M.E) (hX : X ⊆ M.E) (heX : e ∉ M.closure X) :
  ...
  exact hI'.indep.modularPair_of_union.of_isBasis_right hI
...
-- lemma IsFlat.modularPair_iff_foo (hF : M.IsFlat F) (hX : M.IsFlat X) :
