...
lemma isModularFlat_iff_forall_exists_isBasis : M.IsModularFlat X ↔
    M.IsFlat X ∧ ∀ ⦃F⦄, M.IsFlat F → ∃ I, M.Indep I ∧ M.IsBasis (X ∩ I) X ∧ M.IsBasis (F ∩ I) F := by
...
/-- In a simple matroid, being a modular isFlat is the same as being a modular pair with each isFlat. -/
lemma IsModularFlat.IsFlat [Simple M] : M.IsModularFlat X ↔ ∀ ⦃F⦄, M.IsFlat F → M.ModularPair X F := by
  wlog hX : X ⊆ M.E
...
lemma IsFlat.isModularFlat_iff_forall_skew_of_inter (hX : M.IsFlat X) :
    M.IsModularFlat X ↔ ∀ ⦃F⦄, M.IsFlat F → X ∩ F ⊆ M.closure ∅ → M.Spanning (X ∪ F) → M.Skew X F := by
  ...
  obtain ⟨IU, hIU, hIU_eq⟩ := hIY.exists_isBasis_inter_eq_of_superset (Y := X ∪ Y) subset_union_right
...
/-- A hyperplane is a modular isFlat iff it meets every line. -/
lemma Hyperplane.isModularFlat_iff_forall_line {H : Set α} (hH : M.Hyperplane H) :
    M.IsModularFlat H ↔ ∀ L, M.Line L → ¬ (L ∩ H ⊆ M.closure ∅) := by
...
lemma Line.isModularFlat_of_forall_hyperplane {L : Set α} (hL : M.Line L)
    (h : ∀ ⦃H⦄, M.Hyperplane H → ¬ (L ∩ H ⊆ M.closure ∅)) : M.IsModularFlat L := by
...
lemma IsFlat.isModularFlat_iff_forall_contract_exists_parallel (hX : M.IsFlat X) :
    M.IsModularFlat X ↔ ∀ ⦃C : Set α⦄ ⦃e⦄, Disjoint C X → (M ／ C).Nonloop e → e ∈ (M ／ C).closure X →
...
lemma IsModularFlat.inter_insert_closure_point_of_skew (hF : M.IsModularFlat F)
...
/-- The union of a directed family of modular isFlats spans a modular isFlat.
...
lemma IsModularFlat.inter (hX : M.IsModularFlat X) (hY : M.IsModularFlat Y) : M.IsModularFlat (X ∩ Y) := by
...
lemma IsModularFlat.iInter {ι : Type*} [Nonempty ι] [Finitary M] {X : ι → Set α}
  ...
  · intro F hF hfin
    ...
    obtain ⟨i₀, hi₀⟩ := IsFlat.iInter_mem_of_directed_of_isRkFinite (fun i ↦ (hX i).isFlat.inter hF) hdir'
  ...
  -- The collection of finite-rank subisFlats of some `F` is upwards directed and has union `F`.
  -- Since `⋂ i, X i` is a modular pair with every isFlat in the collection, it is modular with `F`.
  ...
  · rw [subset_antisymm_iff, hD_def]
    ...
    · rw [isRkFinite_closure_iff]
      exact ⟨M.closure_isFlat _, M.isRkFinite_of_finite (by simp), hF.closure_subset_of_subset (by simpa)⟩
    simp
...
lemma IsModularFlat.restrict (hF : M.IsModularFlat F) (hFX : F ⊆ X) (hXE : X ⊆ M.E := by aesop_mat) :
    (M ↾ X).IsModularFlat F := by
...
/-- A modular matroid is one where every isFlat is modular. The simple finite modular matroids
...
lemma Modular.modularPair (h : M.Modular) (hF : M.IsFlat F) (hF' : M.IsFlat F') : M.ModularPair F F' :=
  (h hF).modularPair hF'

lemma freeOn_modular (E : Set α) : (freeOn E).Modular := by
  intro F
  simp only [freeOn_isFlat_iff, isModularFlat_iff, modularPair_iff, freeOn_indep_iff, freeOn_isBasis_iff,
    inter_eq_left]
  aesop
...
private lemma exists_of_modular_not_finitary (hM : ∀ L, M.Line L → M.IsModularFlat L)
    (hnotfin : ¬ M.Finitary) :
...
lemma finitary_of_forall_line_modular (hM : ∀ L, M.Line L → M.IsModularFlat L) : M.Finitary := by
  by_contra hnotfin
...
lemma modular_iff_forall_line_hyperplane :
  ...
  · exact hH.isModularFlat_iff_forall_line.1 (h hH.isFlat) L hL
  ...
  · refine finitary_of_forall_line_modular fun L hL ↦ ?_
    exact hL.isModularFlat_of_forall_hyperplane <| fun H hH ↦ h hL hH
  refine IsModularFlat.sInter hne fun H hH ↦ ?_
  rw [(hHs _ hH).isModularFlat_iff_forall_line]
...
lemma Modular.finitary (hM : M.Modular) : M.Finitary :=
  finitary_of_forall_line_modular fun _ hL ↦ hM hL.isFlat
