...
theorem Subtype.exists_eq_coe_image_of_subset {s t : Set α} (h : s ⊆ t) : 
    ∃ (s' : Set t), s = val '' s' :=
  ⟨_, (coe_image_of_subset h).symm⟩ 

@[simp] theorem Subtype.preimage_image_coe (s : Set α) (t : Set s) :
    val ⁻¹' (val '' t) = t :=
  preimage_image_eq t Subtype.coe_injective

@[simp] theorem Subtype.image_coe_inj_iff {r : Set α} {s t : Set r} : 
    val '' s = val '' t ↔ s = t :=
  image_eq_image Subtype.coe_injective

@[simp] theorem Subtype.image_coe_subset_image_coe_iff {r : Set α} {s t : Set r} : 
    val '' s ⊆ val '' t ↔ s ⊆ t :=
...
structure Iso (M₁ : Matroid α₁) (M₂ : Matroid α₂) extends Equiv M₁.E M₂.E where
  on_base' : ∀ (B : Set M₁.E), M₁.Base (val '' B) ↔ M₂.Base (val '' (toFun '' B))

infixl:75 " ≃i " => Matroid.Iso

instance Iso.equivLike {α β : Type _} {M₁ : Matroid α} {M₂ : Matroid β} :
...
def Iso.symm (e : M ≃i N) : N ≃i M where
  toEquiv := (equivLike.toEquiv e).symm
  on_base' := by
    intro B
    rw [e.on_base']
    convert Iff.rfl
    exact ((equivLike.toEquiv e).image_symm_image B)
    
@[simp] theorem coe_symm (e : M ≃i N) : (e.symm : N.E → M.E) = e.toEquiv.symm := rfl
...
def Iso.trans {M₁ : Matroid α₁} {M₂ : Matroid α₂} {M₃ : Matroid α₃} (e₁ : M₁ ≃i M₂)
    (e₂ : M₂ ≃i M₃) : M₁ ≃i M₃
    where
...
def isoOfIndep (e : M.E ≃ N.E)
    (hi : ∀ I : Set M.E, M.Indep (val '' I) ↔ N.Indep (val '' (e '' I))) : M ≃i N
    where
  toEquiv := e
  on_base' := by
    intro B
    simp only [toFun_as_coe_apply] at hi ⊢
    simp_rw [base_iff_maximal_indep, image_subset_iff, ←hi, and_congr_right_iff]
    refine' fun _ ↦ ⟨fun hI I hIN hB' ↦ _, fun hI' I hI hBI ↦ _⟩ 
    · obtain ⟨I, rfl⟩ := exists_eq_coe_image_of_subset hIN.subset_ground
      rw [preimage_image_eq _ coe_injective] at hB' 
      specialize hI (val '' (e ⁻¹' I))
      rw [preimage_image_coe, imp_iff_right hB', image_coe_inj_iff, hi, image_preimage, 
        imp_iff_right hIN] at hI
      rw [hI, image_preimage]
    obtain ⟨I, rfl⟩ := exists_eq_coe_image_of_subset hI.subset_ground
    specialize hI' (val '' (e '' I)) 
    rw [preimage_image_coe] at hBI
    simp only [preimage_image_coe, preimage_image, image_coe_inj_iff, image_eq_iff_eq, 
      imp_iff_right hBI, ← hi, imp_iff_right hI] at hI'
    rw [hI']
    
noncomputable def isoOfBijOn (M : Matroid α) (N : Matroid β) (f : α → β)
    (hbij : BijOn f M.E N.E) (hf : ∀ X, X ⊆ M.E → (M.Base X ↔ N.Base (f '' X))) : M ≃i N
    where
  toEquiv := Set.BijOn.equiv f hbij
  on_base' := by
    intro B
    
    rw [Equiv.toFun_as_coe, hf (val '' B) (Subtype.coe_image_subset (E M) B)]
    convert Iff.rfl
    nth_rw 1 [←preimage_image_coe _ B]
    
    -- rw [preimage_image_coe _ (f '' (val '' B))]
    set B' := val '' B with hB'
    ...
    · rintro ⟨hy, x, hx, hxB, hb⟩
      ...
      simp [MapsTo.restrict] at hb
      rw [← Subtype.coe_inj, Subtype.coe_mk] at hb 
      rw [← hb]
      rfl
    rintro ⟨x, ⟨hx, hB⟩, rfl⟩
    exact ⟨hbij.maps_to hx, x, hx, hB, rfl⟩
#align matroid_in.iso_of_bij_on Matroid.isoOfBijOn
...
theorem Iso.exists_bijOn [Nonempty β] (e : M ≃i N) :
  ...
  exact he
#align matroid_in.iso.exists_bij_on Matroid.Iso.exists_bijOn
...
#align matroid_in.iso_of_bij_on_indep Matroid.isoOfBijOnIndep
...
#align matroid_in.iso.exists_bij_on_indep Matroid.Iso.exists_bijOn_indep

def Iso.image (e : M ≃i N) (B : Set α) : Set β :=
  val '' (e '' (coe ⁻¹' B))
#align matroid_in.iso.image Matroid.Iso.image

def Iso.preimage (e : M ≃i N) (B : Set β) : Set α :=
  val '' (e ⁻¹' (coe ⁻¹' B))
#align matroid_in.iso.preimage Matroid.Iso.preimage

@[ssE_finish_rules]
theorem Iso.image_subset_ground (e : M ≃i N) (X : Set α) : e.image X ⊆ N.E :=
  Subtype.coe_image_subset _ _
#align matroid_in.iso.image_subset_ground Matroid.Iso.image_subset_ground

@[ssE_finish_rules]
theorem Iso.preimage_subset_ground (e : M ≃i N) (X : Set β) : e.Preimage X ⊆ M.E :=
  Subtype.coe_image_subset _ _
#align matroid_in.iso.preimage_subset_ground Matroid.Iso.preimage_subset_ground
...
#align matroid_in.iso.preimage_image Matroid.Iso.preimage_image
...
#align matroid_in.iso.image_preimage Matroid.Iso.image_preimage
...
#align matroid_in.iso.image_inj Matroid.Iso.image_inj
...
theorem Iso.preimage_inj (e : M ≃i N) {X X' : Set β} (hB : X ⊆ N.E) (hB' : X' ⊆ N.E)
  ...
  rwa [iso.preimage, iso.preimage, image_eq_image Subtype.coe_injective,
    ...
    rwa [Subtype.range_coe]
#align matroid_in.iso.preimage_inj Matroid.Iso.preimage_inj
...
theorem Iso.image_eq_preimage_symm (e : M ≃i N) {X : Set α} : e.image X = e.symm.Preimage X :=
  ...
  rfl
#align matroid_in.iso.image_eq_preimage_symm Matroid.Iso.image_eq_preimage_symm
...
theorem Iso.preimage_eq_image_symm (e : M ≃i N) {X : Set β} : e.Preimage X = e.symm.image X :=
  ...
  rfl
#align matroid_in.iso.preimage_eq_image_symm Matroid.Iso.preimage_eq_image_symm
...
#align matroid_in.iso.image_eq_image_inter_ground Matroid.Iso.image_eq_image_inter_ground
...
#align matroid_in.iso.preimage_eq_preimage_inter_ground Matroid.Iso.preimage_eq_preimage_inter_ground
...
theorem Iso.image_ground (e : M ≃i N) : e.image M.E = N.E :=
  ...
  simp
#align matroid_in.iso.image_ground Matroid.Iso.image_ground

@[simp]
theorem Iso.preimage_ground (e : M ≃i N) : e.Preimage N.E = M.E := by
  rw [iso.preimage_eq_image_symm, iso.image_ground]
#align matroid_in.iso.preimage_ground Matroid.Iso.preimage_ground
...
#align matroid_in.iso.image_inter Matroid.Iso.image_inter
...
#align matroid_in.iso.preimage_compl Matroid.Iso.preimage_compl
...
#align matroid_in.iso.image_compl Matroid.Iso.image_compl
...
#align matroid_in.iso.image_diff Matroid.Iso.image_diff

@[simp]
theorem Iso.image_empty (e : M ≃i N) : e.image ∅ = ∅ := by simp [iso.image]
#align matroid_in.iso.image_empty Matroid.Iso.image_empty
...
#align matroid_in.iso.image_subset_image Matroid.Iso.image_subset_image

theorem Iso.image_ground_diff (e : M ≃i N) (X : Set α) : e.image (M.E \ X) = N.E \ e.image X := by
  rw [iso.image_diff, iso.image_ground]
#align matroid_in.iso.image_ground_diff Matroid.Iso.image_ground_diff
...
#align matroid_in.iso.dual Matroid.Iso.dual

@[simp]
theorem Iso.dual_coe (e : M ≃i N) : (e.dual : M﹡.E → N﹡.e) = e :=
  rfl
#align matroid_in.iso.dual_coe Matroid.Iso.dual_coe

@[simp]
theorem Iso.dual_image (e : M ≃i N) : e.dual.image = e.image :=
  rfl
#align matroid_in.iso.dual_image Matroid.Iso.dual_image

@[simp]
theorem Iso.dual_preimage (e : M ≃i N) : e.dual.Preimage = e.Preimage :=
  rfl
#align matroid_in.iso.dual_preimage Matroid.Iso.dual_preimage
...
theorem Iso.on_base (e : M ≃i N) {B : Set α} (hI : B ⊆ M.E) : M.Base B ↔ N.Base (e.image B) :=
  ...
  rfl
#align matroid_in.iso.on_base Matroid.Iso.on_base
...
#align matroid_in.iso.on_indep Matroid.Iso.on_indep
...
-- def iso.image (e : M₁ ≃i M₂) (B : set α₁) : set α₂ := val '' (e '' (coe ⁻¹' B))
-- def iso.preimage (e : M₁ ≃i M₂) (B : set α₂) : set α₁ := val '' (e ⁻¹' (coe ⁻¹' B))
